...
public class AccionesSemanticasIniciador extends
	...
	public AccionesSemanticasIniciador() {
		...
		try {
			...
			ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
					tipoEntidad, "Construyendo agente reactivo " + nombreAgente
		...
		try {
			...
			if (SeHapodidoLocalizarEsquema
				...
				this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
						"existenEntidadesDescripcion", nombreAgente,
						nombreAgente));
			} else {
				this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
						"errorLocalizacionFicherosDescripcion", nombreAgente,
						nombreAgente));
			}
		} catch (Exception e2) {
			e2.printStackTrace();
	...
	public String localizarComportamientoGestorInicial(String identGestor)
		...
		// Se obtiene el gestor inicial de la descripcion de la organizacion.
	...
	public void crearGestorInicial() throws Exception {
		...
		// }
		// else{
		// // El gestor inicial debe ser un gestor de organizaci�n o un gestor
	...
	public void arrancarGestorInicialyTerminar() throws Exception {
		...
		try {
			...
			if (ItfgestGestorInicial != null) {
				...
				this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
						"GestorInicial_arrancado_ok", nombreAgente,
						nombreAgente));
		...
		try {
			this.itfUsoPropiadeEsteAgente
					.aceptaEvento(new EventoRecAgte(
							"imposible_recuperar_arranque", nombreAgente,
							nombreAgente));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void monitorizarAgentes() {
		...
		if (errorEncontrado) {
			try {
				this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
						"error_al_monitorizar", nombreAgente, nombreAgente));
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			try {
				this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
						"agentes_ok", nombreAgente, nombreAgente));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
