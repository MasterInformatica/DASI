...
public class Focus {
	...
	public synchronized void setFoco(Objetivo obj) {
		...
		// A�adido para depurar los objetivos: informo del identificador y la
		// clase de objetivo
		if (obj == null)
			this.foco = null;
		else {
	...
	public synchronized void setfaseProcesoConsecObjetivos(
			String faseProcesoConsec) {
	...
	/**
	 * Refocaliza en el objetivo anterior al actualmente focalizado Elimina de
	 * la pila el foco y lo toma como foco actual
	 * 
	 */
	public synchronized void refocus() {
		if (this.focosAnteriores.peekLast() == null)
			this.setFoco(null);
		else
			this.setFoco(this.focosAnteriores.pollLast());
		// trazas.aceptaNuevaTraza(new
		// InfoTraza("","Foco: Focalizando el objetivo "+foco.getgoalId(),InfoTraza.NivelTraza.debug));
	...
	public synchronized void setFocusToObjetivoMasPrioritario(
	...
	public synchronized void refocusUltimoObjetivoSolving() {
		/**
		 * Devuelve el ultimo objetivo focalizado que sigue en solving . Esto
		 * permite refocalizar en el ultimo Objetivo solving. De paso eliminamos
	...
	public String toString() {
		return "(FOCO: focoActual= " + this.foco + "  focosAnteriores= "
				+ this.focosAnteriores + " )";
	}
