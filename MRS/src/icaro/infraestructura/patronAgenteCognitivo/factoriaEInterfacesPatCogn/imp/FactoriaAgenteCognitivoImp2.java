...
public class FactoriaAgenteCognitivoImp2 extends FactoriaAgenteCognitivo {
	...
	public void createCognitiveAgent(DescInstanciaAgente descInstanciaAgente)
		...
		String nombreAgenteEnMinusculas = descName.substring(0, 1)
				.toLowerCase() + descName.substring(1);
		/*
		 * String ficheroReglas =
		 * NombresPredefinidos.RUTA_PROCESO_RESOLUCION_COGNITIVO; ficheroReglas
		 * =
		 * ficheroReglas.replaceAll(NombresPredefinidos.regex_ruta_automata_package
		 * , nombreAgenteEnMinusculas); ficheroReglas =
		 * ficheroReglas.replaceAll(
		 * NombresPredefinidos.regex_ruta_automata_behaviour, descName);
		 */
		...
		try {
			...
			repositorioInterfaces.registrarInterfaz(NombresPredefinidos.ITF_USO
					+ agentName, (ItfUsoAgenteCognitivo) agent);
			repositorioInterfaces.registrarInterfaz(
					NombresPredefinidos.ITF_GESTION + agentName,
					(InterfazGestion) agent);
	...
	public void crearAgenteCognitivo(DescInstanciaAgente descInstanciaAgente)
		...
		if ((rutaReglas != null)) {
			try {
				...
				// del agente
				ItfUsoAutomataEFsinAcciones itfAutomataEstadoAgente = (ItfUsoAutomataEFsinAcciones) ClaseGeneradoraAutomataEFsinAcciones
						.instance(NombresPredefinidos.FICHERO_AUTOMATA_CICLO_VIDA_COMPONENTE);
				// itfAutomataEstadoAgente.transita(NombresPredefinidos.);
				agenteContrl = (AgenteCognitivo) new AgenteCognitivotImp2(
				...
				repositorioInterfaces
						.registrarInterfaz(NombresPredefinidos.ITF_GESTION
								+ nombreInstanciaAgente,
								(InterfazGestion) agenteContrl);
				repositorioInterfaces.registrarInterfaz(
						NombresPredefinidos.ITF_USO + nombreInstanciaAgente,
						(ItfUsoAgenteCognitivo) agenteContrl);
