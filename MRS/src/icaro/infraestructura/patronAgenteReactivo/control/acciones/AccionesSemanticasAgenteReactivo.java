...
public abstract class AccionesSemanticasAgenteReactivo {
	...
	public void setItfUsoRepositorioInterfaces(
			ItfUsoRepositorioInterfaces itfUsoR) {
	...
	public void informaraMiAutomata(String input, Object[] infoComplementaria) {
		...
		try {
			...
			logger.error("Ha habido un problema enviar un  input a este agente "
					+ nombreAgente);
	...
	protected Object obtenerParametro(String nombre) {
		...
		// else
		System.err.println("Se intenta acceder al parametro: " + nombre
				+ ", pero no estaba definido en la tabla de parametros");
		return null;
	}

	/**
	 * Esta accin no hace nada, sirve para transitar de estado sin ejecutar
	 * nada.
	 */
	public void nula() {
	}
	...
	public void generarTimeOut(long milis, String nombre, String origen,
			String destino) {
		GenerarEventoTimeOut timeout = new GenerarEventoTimeOut(milis, nombre,
				origen, destino, this.itfUsoRepositorio);
		logger.debug("Generando evento de timeout de " + milis
				+ " milisegundos");
		timeout.start();
	}

	public void generarTimeOutInterno(long milis, String nombre, String origen,
			ItfUsoAgenteReactivo itfAgente) {
		GenerarEventoTimeOut timeout = new GenerarEventoTimeOut(milis, nombre,
				origen, itfAgente);
		logger.debug("Generando evento de timeout interno de " + milis
				+ " milisegundos");
		timeout.start();
	}
