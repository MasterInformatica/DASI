...
public class GeneraEstadisticaIntervaloLlegadaYAsignacionVictimas {
	...
	public void CrearFicheroXMLIntervaloLlegadaYAsignacionVictimas(
			String equipo, int numeroRobotsSimulacion,
			int numeroVictimasSimulacion, int intervaloFrecuenciaEnvioVictimas) {
		...
		// String ficheroXMLResultado =
		// icaro.aplicaciones.Rosace.utils.ConstantesRutasEstadisticas.rutaficheroXMLEstadisticasLlegadaYAsignacionVictimas;
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				"yyyy_MM_dd_HH_mm_ss");
		String fecha = dateFormat.format(Calendar.getInstance().getTime());
		String ficheroXMLResultado = "estadisticas/"
		...
		// ReadXMLTestRobots(rutaFicheroRobotsSimulacion);
		// Document robotsSimulacion =
		// rXMLTRobots.getDocument(rutaFicheroRobotsSimulacion);
		// NodeList listaNodosRobotsSimulacion =
		// rXMLTRobots.getRobotsXMLStructure(robotsSimulacion, "robot");
		...
		try {
			...
			pwficheroXMLResultado.print("simulacion " + equipo
					+ " ; numero robots simulacion = " + numeroRobotsSimulacion
					+ " ; numero victimas simulacion = "
					+ numeroVictimasSimulacion + " ; frecuencia de envio = "
					+ intervaloFrecuenciaEnvioVictimas + " milisegundos");
		...
		try {
			...
			docficheroXMLLlegadaYAsignacionVictimas = dBuilder.parse(new File(
					ficheroXMLLlegadaVictimasYAsignacionVictimas));
			docficheroXMLLlegadaYAsignacionVictimas.getDocumentElement()
					.normalize();
		} catch (Exception e) {
			e.printStackTrace();
		}

		NodeList nodeLstficheroXMLLlegadaYAsignacionVictimas = docficheroXMLLlegadaYAsignacionVictimas
				.getElementsByTagName("item");
		int nroItemsficheroXMLLlegadaYAsignacionVictimas = nodeLstficheroXMLLlegadaYAsignacionVictimas
				.getLength();

		System.out.println("nroItemsficheroXMLLlegadaYAsignacionVictimas->"
				+ nroItemsficheroXMLLlegadaYAsignacionVictimas);
		...
		for (int i = 1; i <= totalVictimasEnviadas; i++) {
			...
			String stri = i + "";
			tiempoLlegadaVictima = buscaTiempoLlegadaVictimaEnviadaI(
					nodeLstficheroXMLLlegadaYAsignacionVictimas, stri);

			System.out.println("tiempoLlegadaVictima " + i + "->"
					+ tiempoLlegadaVictima);

			tiempoAsignacionVictima = buscaTiempoAsignacionVictimaEnviadaI(
					nodeLstficheroXMLLlegadaYAsignacionVictimas, stri);

			System.out.println("tiempoAsignacionVictima " + i + "->"
					+ tiempoAsignacionVictima);

			intervaloLlegadaAsignacion = tiempoAsignacionVictima
					- tiempoLlegadaVictima;

			System.out.println("intervaloLlegadaAsignacion " + i + "->"
					+ intervaloLlegadaAsignacion);

			strIntervaloLlegadaAsignacion = intervaloLlegadaAsignacion + "";
			strIntervaloLlegadaAsignacion = ObtenerTiempoEnSegundosNDecimales(
					strIntervaloLlegadaAsignacion, 1); // 1 decimal
			System.out.println("strIntervaloLlegadaAsignacion " + i + "->"
					+ strIntervaloLlegadaAsignacion + "\n");

			// escribir en el fichero un item
			escribeEstadisticaFicheroXMLIntervaloLlegadaYAsignacionVictimas(
					pwficheroXMLResultado, stri, strIntervaloLlegadaAsignacion);
		}
		...
		// Crear y mostrar las series de estadisticas recogidas en graficas del
		// jfreechart
		// pwficheroXMLResultado.print("simulacion " + equipo +
		// " ; numero robots simulacion = " + numeroRobotsSimulacion +
		// " ; numero victimas simulacion = " + numeroVictimasSimulacion +
		// " ; frecuencia de envio = " + intervaloFrecuenciaEnvioVictimas +
		// " milisegundos");

		String cadena = equipo + ": " + numeroRobotsSimulacion + " robots"
				+ " ; " + numeroVictimasSimulacion + " victimas" + " ; "
				+ intervaloFrecuenciaEnvioVictimas + " milisegundos";

		visualizadorJFchart = new VisualizacionJfreechart(
				"Graficas estadisticas simulacion " + cadena);
		...
		visualizadorJFchart.addSerie(1, Color.green,
				serieIntervaloLlegadaYAsignacionVictimas);

		System.out.println("Fichero de estadisticas salvado en "
				+ ficheroXMLResultado);
		// String text = "Fichero de estadisticas salvado en " +
		// ficheroXMLResultado;
		// String title =
		// "Fin Generacion Estadistica IntervaloLlegadaYAsignacion";
	...
	private int buscaTotalVictimasEnviadas(
			NodeList nodeLstficheroXMLLlegadaYAsignacionVictimas) {
		int contador = 0;
		...
		for (int i = 0; i < nroItemsficheroXMLLlegadaYAsignacionVictimas; i++) {
			infofichaItem = ObtenerElementoItem(
					nodeLstficheroXMLLlegadaYAsignacionVictimas, i);
			// obtener valor tiempo de nrovictimasenentorno
			nrovictimasenentorno = ObtenerElementoItem(infofichaItem,
	...
	private double buscaTiempoLlegadaVictimaEnviadaI(
			NodeList nodeLstficheroXMLLlegadaYAsignacionVictimas,
			String nroVictima) {
		...
		for (int i = 0; i < nroItemsficheroXMLLlegadaYAsignacionVictimas; i++) {
			...
			if (nrovictimasenentorno.equals(nroVictima)) {
				strtiempoResultado = ObtenerElementoItem(infofichaItem,
						"tiempo");
				tiempoResultado = Double.valueOf(strtiempoResultado)
						.doubleValue();
				i = nroItemsficheroXMLLlegadaYAsignacionVictimas;// salir del
																	// for
			}
		}
	...
	private double buscaTiempoAsignacionVictimaEnviadaI(
			NodeList nodeLstficheroXMLLlegadaYAsignacionVictimas,
			String nroVictima) {
	...
	private void escribeEstadisticaFicheroXMLIntervaloLlegadaYAsignacionVictimas(
			PrintWriter pw, String NroVictima, String tiempo) {
		String ident1 = "   ";
		String cadena = "<item>" + "\n";
	...
	// de una cadena con valor en segundos
	public String ObtenerTiempoEnSegundosNDecimales(String strSegundos,
			int nrodecimales) {
		// int numtiempoEnMiliSegundos = Integer.parseInt(strMilisegundos);

		// double numtiempoEnSegundos =
		// Double.valueOf(strSegundos).doubleValue();
	...
	public static void main(String[] args) {
		...
		// GeneraEstadisticaIntervaloLlegadaYAsignacionVictimas gEstaInter = new
		// GeneraEstadisticaIntervaloLlegadaYAsignacionVictimas();
