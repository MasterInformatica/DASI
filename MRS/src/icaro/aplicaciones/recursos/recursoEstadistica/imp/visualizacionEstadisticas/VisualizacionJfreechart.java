...
public class VisualizacionJfreechart extends JFrame {
	...
	public void inicializacionJFreeChart(String title, String xAxisLabel,
			String yAxisLabel, PlotOrientation orientation, boolean legend,
			boolean tooltips, boolean urls) {
		chart1 = ChartFactory.createXYLineChart(title, // chart title Titulo
														// local del grafico
				xAxisLabel, // x axis label
				yAxisLabel, // y axis label
				null, // data
				orientation, legend, // include legend
				tooltips, // tooltips
				urls // urls
				);
	...
	public void setColorChartBackgroundPaint(Color c) {
		chart1.setBackgroundPaint(c); // El fondo exterior del grafico sera del
										// color pasado por parametro
	}

	public void setColorChartPlotBackgroundPaint(Color c) {
		plot = chart1.getXYPlot();
		plot.setBackgroundPaint(c); // El fondo interior del grafico sera del
									// color pasado por parametro
	}

	public void setColorChartPlotDomainGridlinePaint(Color c) {
		plot = chart1.getXYPlot();
		plot.setDomainGridlinePaint(c); // Las lineas verticales de la
										// cuadricula se pinta de color pasado
										// por parametro
	}

	public void setColorChartPlotRangeGridlinePaint(Color c) {
		plot = chart1.getXYPlot();
		plot.setRangeGridlinePaint(c); // Las lineas horizontales de la
										// cuadricula se pintan de color pasado
										// por parametro
	}

	public void inicializacionJFreeChart() {
		// Creates a SegmentedTimeline instance, the time scope is
		// "09:30-11:30，13:00-15:00".
		chart1 = ChartFactory.createXYLineChart(
				"Victim's Notification and Assignment to Team members ", // chart
																			// title
																			// Titulo
																			// local
																			// del
		...
		chart1.setBackgroundPaint(Color.white); // El fondo exterior del grafico
												// sera blanco
		// chart1.setBackgroundPaint(Color.green);

		// get a reference to the plot for further customisation...
		plot = chart1.getXYPlot();
		plot.setBackgroundPaint(Color.lightGray); // El fondo interior del
													// grafico sera gris
		// plot.setBackgroundPaint(Color.blue);

		plot.setDomainGridlinePaint(Color.white); // Las lineas verticales de la
													// cuadricula se pinta de
													// color blanco
		plot.setRangeGridlinePaint(Color.white); // Las lineas horizontales de
													// la cuadricula se pintan
													// de color blanco
	...
	// por el momento no me hace caso para personalizar el aspecto de la serie
	// (aspecto del punto, color de los puntos, puntos unidos con lineas o no,
	// ....)
	public void addSerie(int indexSerie, Color color, XYSeries localXYSeries) {
		...
		Ellipse2D.Double localDouble = new Ellipse2D.Double(-4.0D, -4.0D, 8.0D,
				8.0D); // Forma de la anotacion del punto x,y

		XYLineAndShapeRenderer localXYLineAndShapeRenderer = new XYLineAndShapeRenderer();

		localXYLineAndShapeRenderer.setSeriesLinesVisible(indexSerie, true); // provoca
																				// que
																				// se
																				// pinten
																				// lineas
																				...
																				// la
																				// serie
		localXYLineAndShapeRenderer.setSeriesShapesVisible(indexSerie, false); // provoca
																				// que
																				// se
																				// pinte
																				// la
		...
		plot.setDataset(indexSerie, localXYSeriesCollection); // Se aniade la
																// serie al plot
		plot.setRenderer(indexSerie, localXYLineAndShapeRenderer);

		localXYLineAndShapeRenderer.setSeriesShape(indexSerie,
		...
		// localXYLineAndShapeRenderer.setSeriesFillPaint(indexSerie,
		// Color.yellow);
		// localXYLineAndShapeRenderer.setSeriesOutlinePaint(indexSerie,
		// Color.gray);
	...
	public void VisualizarEstadisticasLLegadayAsignacionVictimas(
			XYDataset dataset) {
		// XYDataset xyDataset = new XYSeriesCollection(serieDatos);
