...
public class ControladorVisualizacionSimulRosace {
	...
	private int intervaloSecuencia = 10000; // valor por defecto. Eso deberia
	...
	private String rutassrc = "src/"; // poner "src/main/java" si el proyecto de
										// icaro se monta en un proyecto maven
	...
	private String directorioPersistencia = VocabularioRosace.NombreDirectorioPersistenciaEscenarios
			+ File.separator;
	...
	public ControladorVisualizacionSimulRosace(
			NotificadorInfoUsuarioSimulador notificadorInfoUsuarioSimulador) {
		notifEvts = notificadorInfoUsuarioSimulador;

	}

	public void setIftRecPersistencia(
			ItfUsoRecursoPersistenciaEntornosSimulacion itfPersisSimul) {
		itfPersistenciaSimul = itfPersisSimul;
	}

	public void initModelosYvistas() {
		// String directorioPersistencia =
		// VocabularioRosace.IdentDirectorioPersistenciaEscenarios+File.separator;
		...
		try {
			...
			// setIdentsEscenarios =
			// persistenciaLocal.obtenerIdentsEscenarioSimulacion(directorioPersistencia);
			// }else
			setIdentsEscenarios = itfPersistenciaSimul
					.obtenerIdentsEscenarioSimulacion();
	...
	public synchronized void peticionComenzarSimulacion(
			String identEscenarioActual, int intervaloSecuencia) {
		...
		else if (identEscenarioActual == null)
			visorControlSim.setIdentEscenarioActual(escenarioEdicionComp
					.getIdentEscenario());
		else if (intervaloSecuencia <= 0)
			visorControlSim.solicitarDefinicionItervaloSecuencia();
		else
			notifEvts
					.sendPeticionSimulacionSecuenciaVictimasToRobotTeam(intervaloSecuencia);
	}
	...
	public synchronized void peticionCrearEscenario() {
		...
		// RobotInfoEnEscenario1 infoRobot = (RobotInfoEnEscenario1)
		// escenarioActualComp.getRobotInfo(ideRobot);
		// List<RobotCapability> capacidades=infoRobot.getRobotCapabilities();
		// System.out.println("Despues de  la visualizacion Lista de capacidades a guardar del robot  : "
		// + ideRobot+" Capacidades : "+ capacidades.toString() );
		// }
	}
	...
	public void peticionAbrirEscenario() {
		...
		// visorControlSim.solicitarSeleccionFichero(directorioPersistencia);
		if (!visorControlSim.hayFicherosCreados()) {
			this.visorControlSim
					.visualizarConsejo("Sin Escenarios de simulacion",
							"No hay escenarios creados",
							"Abrimos el editor de escenarios para definir Robots y Victimas");
			peticionCrearEscenario();
		} else {
			File ficheroSeleccionado = visorControlSim
					.solicitarSeleccionFichero();
			if (ficheroSeleccionado == null)
				visorControlSim.visualizarConsejo(
						tituloAvisoEscenarioNoDefinido,
						mensajeEscenarioNoSeleccionado,
						recomendacionDefinirEscenario);
			else {
				try {
					escenarioEdicionComp = itfPersistenciaSimul
							.obtenerInfoEscenarioSimulacion(ficheroSeleccionado
									.getName());
				} catch (Exception ex) {
					Exceptions.printStackTrace(ex);
					System.out
							.println("Desde peticion Abrir escenario . No se encuetra el fichero Ident Fichero : "
				...
				if (identsRobotsEquipo != null)
					visorControlSim
							.visualizarIdentsEquipoRobot(identsRobotsEquipo);
			}
		}
	}
	...
	public static void main(String args[]) {
		...
		new ControladorVisualizacionSimulRosace(
				new NotificadorInfoUsuarioSimulador("prueba1", "agente2"))
				.initModelosYvistas();
	...
	public void peticionGuardarEscenario(
			EscenarioSimulacionRobtsVictms escenarioComp) {
		// throw new UnsupportedOperationException("Not supported yet."); //To
		// change body of generated methods, choose Tools | Templates.
		// si no hay un escenario actual definido indicamos al usuario que no
		// hay escenario definido
		...
		if (escenarioEdicionComp.getNumRobots() <= 0)
			visorEditorEscen.visualizarConsejo(
					tituloAvisoEscenSinRobotsDefinidos,
					mensajeEscenarioSinRobots, recomendacionDefinirRobots);
		else {
			int respuesta = visorEditorEscen
					.confirmarPeticionGuardarEscenario("Se va a guardar el escenario : ");
			if (respuesta == JOptionPane.OK_OPTION) {

				ArrayList<String> robotNombres = escenarioComp
						.getListIdentsRobots();
				for (String ideRobot : robotNombres) {
					// String ideRobot = (String)robtIter.next();
					RobotStatus1 infoRobot = (RobotStatus1) escenarioComp
							.getRobotInfo(ideRobot);
					List<RobotCapability> capacidades = infoRobot
							.getRobotCapabilities();
					System.out
							.println("Desde peticion Guardar Lista de capacidades a guardar del robot  : "
									+ ideRobot
									+ "Capacidades : "
									+ capacidades.toString());
				}
				System.out
						.println("Desde peticion Guardar Numero de Robots  : "
								+ escenarioEdicionComp.getNumRobots()
								+ " Numero de victimas : "
								+ escenarioEdicionComp.getNumVictimas());
				// if (itfPersistenciaSimul==null)
				// persistenciaLocal.guardarInfoEscenarioSimulacion(directorioPersistencia,
				// escenarioEdicionComp);
				// else
				try {
					itfPersistenciaSimul
							.guardarInfoEscenarioSimulacion(escenarioEdicionComp);
				} catch (Exception ex) {
					Exceptions.printStackTrace(ex);
				}
				if (peticionObtenerEscenarioValido && !escenarioValidoObtenido) {
					// se envia el escenario al agente controlador que puede
					// estar esperandolo
					notifEvts
							.sendInfoEscenarioSeleccionado(escenarioEdicionComp);
				}
			}
			if (!visorMovientoIniciado) {
				visorControlSim.visualizarIdentsEquipoRobot(escenarioComp
						.getListIdentsRobots());
				visorControlSim.setIdentEscenarioActual(escenarioComp
						.getIdentEscenario());
			}
		}
	}
		...
		if (escenarioComp != null) {
			try {
				...
				if (visorMovimientoEscen == null)
					visorMovimientoEscen = new VisorMovimientoEscenario(
							escenarioComp);
				else
					visorMovimientoEscen.visualizarEscenario(escenarioComp);
	...
	public boolean abrirVisorConEscenario(String identFicheroEscenarioSimulacion) {
		...
		if (!visorControlSimuladorIniciado) {
			// initModelosYvistas();
			if (identFicheroEscenarioSimulacion == null) { // Se abre el visor
															// sin
				visorEditorEscen.setVisible(true);
			}
			if (identFicheroEscenarioSimulacion != null)
				try {
					escenarioEdicionComp = itfPersistenciaSimul
							.obtenerInfoEscenarioSimulacion(identFicheroEscenarioSimulacion);
				} catch (Exception ex) {
					Exceptions.printStackTrace(ex);
				}
			if (escenarioEdicionComp == null) {
				this.visorControlSim
						.visualizarConsejo("fichero no encontrado ",
								"verifique el identificador del fichero ",
								"Puede abrir  un escenario existente o crear un nuevo escenario");
				return false;
			}
			visorControlSim
					.setIdentEscenarioActual(identFicheroEscenarioSimulacion);
			visorControlSim.visualizarIdentsEquipoRobot(escenarioEdicionComp
					.getListIdentsRobots());
	...
	public boolean abrirVisorMovimientoConEscenario(
			String identFicheroEscenarioSimulacion) {
		// throw new UnsupportedOperationException("Not supported yet."); //To
		// change body of generated methods, choose Tools | Templates.
		if (!visorMovientoIniciado) {
			try {
				...
				escenarioEdicionComp = itfPersistenciaSimul
						.obtenerInfoEscenarioSimulacion(identFicheroEscenarioSimulacion);
				if (escenarioEdicionComp == null) {
					this.visorControlSim.visualizarConsejo(
							"fichero no encontrado ",
							"verifique el identificador del ficero", null);
					return false;
				}
				visorMovimientoEscen = new VisorMovimientoEscenario(
						escenarioEdicionComp);
	...
	public void peticionVisualizarIdentsRobots(ArrayList identList) {
		// throw new UnsupportedOperationException("Not supported yet."); //To
		// change body of generated methods, choose Tools | Templates.
		// si los identificadores recibidos son diferentes a los que hay en el
		// escenario se cambian los del escenario
		// esto es un poco raro pero resuelve temporalmente el problema
		// cambiar los nombres en el computacional y tambien las etiquetas en el
		// visualizador
		...
		if (identList != null) {
			...
			if (escenarioEdicionComp.getListIdentsRobots().size() != identList
					.size())
				visorControlSim
						.visualizarConsejo(
								"Imposible cambiar los identificadores",
								"El numero de robots en el escenario actual y el recibido es diferente",
								"revisar las definiciones de los escenarios");

			else {
				this.escenarioEdicionComp.renombrarIdentRobts(identList);
				ArrayList<String> robotNombres = escenarioEdicionComp
						.getListIdentsRobots();
				for (String ideRobot : robotNombres) {
					// String ideRobot = (String)robtIter.next();
					RobotStatus1 infoRobot = (RobotStatus1) escenarioEdicionComp
							.getRobotInfo(ideRobot);
					// List<RobotCapability>
					// capacidades=infoRobot.getRobotCapabilities();
					System.out
							.println("Desde peticionVisualizarIdentsRobots  nuevo ident Robot : "
									+ ideRobot);
				}
				Object[] victimasIdents = escenarioEdicionComp.getVictims()
						.keySet().toArray();
				for (int i = 0; i < victimasIdents.length; i++) {
					String idVictima = (String) victimasIdents[i];
					System.out
							.println("Desde peticionVisualizarIdentsRobots  nuevo ident Vicitima : "
									+ idVictima);
				}
	...
	public void peticionSalvarVictima() {
		// throw new UnsupportedOperationException("Not supported yet."); //To
		// change body of generated methods, choose Tools | Templates.
		// verificar si hay un escenario de simulacion seleccionado valido y los
		// robots estan visualizados
		// obtener las victimas del escenario, se visualizan y se le pide al
		// usuario que seleccione una
		// situaciones a anotar : escenario valido seleccionado, robots en el
		// escenario visualizados,victimas visualizadas
		if (robotsVisualizados && escenarioValidoObtenido) {
			...
			visorControlSim.visualizarConsejo("Seleccion de Victima",
					"Debe seleccionar la victima a rescatar ",
					"Haga doble clic en el identificador de la victima");
		}

		System.out.println("Desde peticionVisualizarIdentsRobots   : "
				+ escenarioValidoObtenido + robotsVisualizados);
	}

	public boolean verificarCaracteristicasEscenarioAbierto(String orgModelo,
			int numRobots) {
		return (this.numeroRobots == numRobots)
				&& (this.modeloOrganizativo.equalsIgnoreCase(orgModelo));
	}

	public boolean verificarCaracteristicasEscenarioSeleccionado(
			File ficheroSeleccionado, String orgModelo, int numRobots) {
		try {
			escenarioEdicionComp = itfPersistenciaSimul
					.obtenerInfoEscenarioSimulacion(ficheroSeleccionado
							.getName());
		...
		if (this.numeroRobots == numRobots
				&& this.modeloOrganizativo.equalsIgnoreCase(orgModelo)) {
			// se envia notificacion al agente controlador con el computacional
			// obtenido
			escenarioEdicionComp.setGestorEscenarios(gestionEscComp);
			...
			visorControlSim.visualizarConsejo(
					"Fichero seleccionado No valido ",
					"El modelo organizativo del fichero seleccionado: "
							+ orgModelo + " o el  numero de Robots = : "
							+ numRobots + " No coinciden ",
					"Seleccione otro fichero o  cree uno nuevo ");
			return false;
		}
	}
	...
	public void peticionObtenerEscenarioSimulacion(String modOrganizativo,
			int numRobots) {
		// throw new UnsupportedOperationException("Not supported yet."); //To
		// change body of generated methods, choose Tools | Templates.
		// casos
		// caso 1 no hay ningun escenario abierto se informa al usuario que debe
		// seleccionar o crear un escenario para poder hacer la simulacion
		if (escenarioActualAbierto) {
			if (this.verificarCaracteristicasEscenarioAbierto(modOrganizativo,
					numRobots))
				if (visorControlSim
						.solicitarConfirmacion("Desea utilizar el escenario actual abierto como escenario de simulacion ?"))
					notifEvts
							.sendInfoEscenarioSeleccionado(escenarioEdicionComp);
		}
		...
		if (!visorControlSim.hayFicherosCreados()) {
			visorControlSim.visualizarConsejo("Creacion de Fichero ",
					" No hay ficheros de simulacion creados",
					"Debe crear un fichero de modelo organizativo: "
							+ modOrganizativo + " y numero de Robots = : "
							+ numRobots);
			...
			visorControlSim
					.visualizarConsejo("Seleccion de Fichero ",
							"Seleccionar un fichero de modelo organizativo: "
									+ modOrganizativo
									+ " y numero de Robots = : " + numRobots,
							"Si no tiene ninguno de estas caracteristicas cree uno nuevo ");
			File ficheroSeleccionado = this.peticionSeleccionarEscenario();
			int numeroIntentos = 0;
			boolean ficheroSeleccionadoValido = false;
			while (numeroIntentos < maxIntentosPeticionSeleccionEscenario
					&& !ficheroSeleccionadoValido) {
				if (ficheroSeleccionado != null)
					if (!verificarCaracteristicasEscenarioSeleccionado(
							ficheroSeleccionado, modOrganizativo, numRobots)) {
						visorControlSim
								.visualizarConsejo("Fichero no valido ",
										"Se debe seleccionar un fichero de modelo organizativo: "
												+ modOrganizativo
												+ " y numero de Robots = : "
												+ numRobots,
										"Si no tiene ninguno de estas caracteristicas cree uno nuevo ");
						ficheroSeleccionado = this
								.peticionSeleccionarEscenario();
						// Se podrian considerar mas intentos pero lo dejamos
						// aqui
					...
					visorControlSim
							.visualizarConsejo("Sin seleccion de fichero ",
									"Se debe seleccionar un fichero de modelo organizativo: "
											+ modOrganizativo
											+ " y numero de Robots = : "
											+ numRobots,
									"Sin fichero que defina el escenario no se puede iniciar la simulacion ");
				numeroIntentos++;
			}
			if (ficheroSeleccionadoValido) {
				...
				notifEvts
						.sendNotifAgteControlador("escenarioNoDefinidoTrasAgotarLosIntentos");
		}

		// caso 3 un escenario abierto y el suario pulsa la opcion simular.
		// es necesario validarlo y si es valido se envia una notificacion al
		// agente con el computacional
		// si no es v�lido hay que pedir que se seleccione otro : noticar que el
		// escenario tiene que tener el mismo
		// numero de robots que el definido en la configuracion y esperar que
		// seleccione o cree uno nuevo
		// una vez conseguido el escenrio valido se cambian los identificadores
		// de los robots para ajustarlos a la configuracion

	}

	public File peticionSeleccionarEscenario() {
		// suponemos que hay escenarios creados , auque lo podemos verificar de
		// nuevo
		int numerointentos = 0;
	...
	// visorControlSim.visualizarConsejo("Seleccion de Fichero ",
	// "Seleccionar un fichero de modelo organizativo: "+modOrganizativo
	// + " y numero de Robots = : "+ numRobots,
	// "Si no tiene ninguno de estas caracteristicas cree uno nuevo ");
	// }

	void notifRecomendacionUsuario(String titulo, String motivo,
	...
	void victimaSeleccionadaParaSimulacion(String identVictimaSeleccionada) {
		...
		if (!avisoSeleccionVictima) {
			this.visorControlSim
					.solicitarConfirmacion(" Se va a proceder a simular salvar a la victima : "
							+ identVictimaSeleccionada);
		}
		this.avisoSeleccionVictima = true;
		this.notifEvts
				.sendPeticionSimulacionVictimToRobotTeam(identVictimaSeleccionada);
	}

	void peticionCambiarPosicionRobot(String identRobot, Integer coordX,
			Integer coordY) {
		this.visorMovimientoEscen.cambiarPosicionRobot(identRobot, coordX,
				coordY);
	}

	void peticionMostrarVictimaRescatada(String identVictima) {
		this.visorMovimientoEscen.cambiarIconoEntidad(identVictima, rutaIconos
				+ imageniconoMujerRescatada);

	}

	void abrirVisorConEscenarioComp(
			EscenarioSimulacionRobtsVictms escenarioSimulacion) {
		if (!visorControlSimuladorIniciado) {
			// initModelosYvistas();
		...
		if (escenarioSimulacion != null) {
			...
			visorControlSim.setIdentEscenarioActual(escenarioSimulacion
					.getIdentEscenario());
			visorControlSim.visualizarIdentsEquipoRobot(escenarioSimulacion
					.getListIdentsRobots());
			visorControlSim.visualizarIdentsVictimas(escenarioSimulacion
					.getListIdentsVictims());
			escenarioValidoObtenido = true;
	...
	// private EscenarioSimulacionRobtsVictms obtenerEscenarioDesdePersistencia
	// (String identFichero){
	// try {
	// if(itfPersistenciaSimul == null){
