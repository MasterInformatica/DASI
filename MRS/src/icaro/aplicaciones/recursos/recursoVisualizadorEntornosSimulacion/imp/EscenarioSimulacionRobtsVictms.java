...
public class EscenarioSimulacionRobtsVictms {
	...
	public synchronized void setGestorEscenarios(
			GestionEscenariosSimulacion gestorEsc) {
		migestor = gestorEsc;
	}
	...
	public synchronized void addVictimLoc(String idVictima,
			Point puntoEnEscenario) {

		if (ultimaVictimaModificada != null
				&& ultimaVictimaModificada.getName()
						.equalsIgnoreCase(idVictima))
	...
	public void addRoboLoc(String idRobot, Point robotLoc) {
		...
		if (ultimoRobotDefinido != null
				&& ultimoRobotDefinido.getIdRobot().equals(idRobot))
	...
	public void eliminarEntidad(String idEntidad) {
		if (idEntidad.contains("Robot")) {
			if (infoRobots.containsKey(idEntidad)) {
				...
				numRobots--;
			} else if (numRobots > 0 && idEntidad.startsWith("init"))
				infoRobots.remove(idEntidad);
			...
			if (infoVictimas.containsKey(idEntidad)) {
				...
				numVictimas--;
			} else if (idEntidad.startsWith("init"))
				infoVictimas.remove(idEntidad);
			if (numVictimas == 0)
				infoVictimas.put(victimInicialId, new Victim(victimInicialId));
		}
	}
	...
	public synchronized String getIdentEscenario() {
		...
		if (identEscenario == null && migestor != null)
			return migestor.getIdentEscenario(modeloOrganizativo, numRobots,
					numVictimas);
	...
	public synchronized void setIdentificadorNormalizado() {
		...
		this.identEscenario = migestor.getIdentEscenario(modeloOrganizativo,
				numRobots, numVictimas);
