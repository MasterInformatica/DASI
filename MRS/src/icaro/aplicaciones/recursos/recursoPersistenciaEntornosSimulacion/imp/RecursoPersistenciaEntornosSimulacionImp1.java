 ...
 * @author FGarijo
 */
public class RecursoPersistenciaEntornosSimulacionImp1 extends
		ImplRecursoSimple {
	...
	private int numeroVictimasDiferentesSimulacion = 0; // Numero de victimas
	...
	private Map<String, String> victimsDiferentesXML = new HashMap<String, String>(); // Victimas
																						...
																						// como
																						// entrada
	private ArrayList<Victim> victimasDefinidas; // Victimas que hay en el
													// entorno
	private ArrayList<RobotStatus> robotsDefinidos; // Victimas que hay en el
													// entorno
	...
	public RecursoPersistenciaEntornosSimulacionImp1(String recursoId)
	...
	public void inicializarRecursoPersistenciaEntornosSimulacion() {
		try {
			// ItfUsoConfiguracion itfconfig = (ItfUsoConfiguracion)
			// itfUsoRepositorioInterfaces.obtenerInterfaz(NombresPredefinidos.NOMBRE_ITF_USO_CONFIGURACION);
			// rutaFicheroVictimasTest =
			// itfconfig.getValorPropiedadGlobal(VocabularioRosace.identRutaFicheroEscenarioSimulacion);
			// rutaFicheroRobotsTest =
			// itfconfig.getValorPropiedadGlobal(VocabularioRosace.rutaFicheroRobotsTest);
			...
			// rXMLTSeq.getNumberOfVictimsInSequence(nodeLst);
			...
			identFicheroInfoAsigVictimasObj = VocabularioRosace.NombreFicheroSerieInfoAsignacionVictimas
					+ ".tmp";
			identFicheroInfoAsigVictimasXML = VocabularioRosace.NombreFicheroSerieInfoAsignacionVictimas
					+ ".xml";
			directorioFicherosPersistencia = new File(directorioPersistencia);
			if (!directorioFicherosPersistencia.exists())
				directorioFicherosPersistencia.mkdir();
			rutaFicherosPersistencia = directorioFicherosPersistencia
					.getAbsolutePath();
			...
			// pruebas
			// File f =
			// obtenerFicheroEscenario("modeloOrg_JerarquicoNumRobts_5NumVicts_5");
			// HashSet set =
			// obtenerIdentsEscenarioSimulacion(directorioPersistencia);
			// EscenarioSimulacionRobtsVictms escenario =
			// obtenerInfoEscenarioSimulacion("modeloOrg_JerarquicoNumRobts_5NumVicts_5");
			// this.guardarInfoEscenarioSimulacion(escenario);
			// robotsDefinidos = getTeamRobotStatus();
		} catch (Exception ex) {
			Logger.getLogger(
					RecursoPersistenciaEntornosSimulacionImp1.class.getName())
					.log(Level.SEVERE, null, ex);
			// return null;
		}
	}

	public boolean guardarInfoEscenarioSimulacion(
			EscenarioSimulacionRobtsVictms escenario) {
		Serializer serializer = new Persister();
		// String identFichero =
		// VocabularioRosace.rutaPersistenciaEscenarios+File.separator+escenario.getIdentEscenario()+".xml";
		try {
			String identFichero = escenario.getIdentEscenario() + extFicheroXML;

			// File ficheroEscenario =
			// this.obtenerFicheroEscenario(identFichero);
			if (identsEscenarioSimulacion.contains(identFichero)) {// Se
																	// renombra
																	// el
																	// fichero
																	// agnadiendo
			...
			identFichero = rutaFicherosPersistencia + File.separator
					+ identFichero;
			...
			for (String ideRobot : robotNombres) {
				...
				RobotStatus1 infoRobot = (RobotStatus1) escenario
						.getRobotInfo(ideRobot);

				List<RobotCapability> capacidades = infoRobot
						.getRobotCapabilities();
				System.out
						.println(" Desde persistencia Lista de capacidades a guardar del robot  : "
								+ ideRobot
								+ "Capacidades : "
								+ capacidades.toString());
			}
			System.out.println("Desde peticion Guardar Numero de Robots  : "
					+ escenario.getNumRobots() + " Numero de victimas : "
					+ escenario.getNumVictimas());

			// serializer.write(escenario, new
			// File(rutaFicheroInfoPersistencia+identFichero+".xml"));
	...
	public EscenarioSimulacionRobtsVictms obtenerInfoEscenarioSimulacion(
			String identFicheroInfoPersistencia) {
		try {
			File ficheroEscenario = this
					.obtenerFicheroEscenario(identFicheroInfoPersistencia);

			if (ficheroEscenario == null) {
				// dirFicherosPersistencia.mkdir();
				// rutaFicheroInfoPersistencia=
				// dirFicherosPersistencia.getAbsolutePath()+"\\";
				System.out.println("El fichero   : "
						+ identFicheroInfoPersistencia + " No existe ");

			} else {
				Serializer serializer = new Persister();
				// EscenarioSimulacionRobtsVictms escenario =
				// serializer.read(EscenarioSimulacionRobtsVictms.class,ficheroEscenario,
				// false);
				return serializer.read(EscenarioSimulacionRobtsVictms.class,
						ficheroEscenario, false);
				// serializer.read(EscenarioSimulacionRobtsVictms.class,ficheroEscenario,
				// false);
	...
	public boolean eliminarEscenarioSimulacion(
			String identFicheroInfoPersistencia) {
		try {
			File ficheroEscenario = this
					.obtenerFicheroEscenario(identFicheroInfoPersistencia);
	...
	public HashSet obtenerIdentsEscenarioSimulacion(
			String identDirectorioInfoPersistencia) {

		if (identsEscenarioSimulacion.isEmpty()) {
			try {
				if (ficherosEscenarios == null)
					return null;
				String identFichero;
	...
	private String renombrarFichero(String identfichero) {
		...
		for (int i = 0; i < ficherosEscenarios.length; i++) {
			if (ficherosEscenarios[i].getName().contains(idFicheroSinXml))
				indiceRepeticion++;
		}
	...
	private File obtenerFicheroEscenario(String identEscenario) {
		...
		if (!identEscenario.endsWith(extFicheroXML))
			identEscenario = identEscenario + extFicheroXML;
		try {
			if (ficherosEscenarios == null)
				return null;
			String identFichero;
			...
			while (i < ficherosEscenarios.length && !encontrado) {
				identFichero = ficherosEscenarios[i].getName();
				if (identFichero.equals(identEscenario))
					encontrado = true;
				else
					i++;
				System.out.println(" Fichero obtenido : " + identFichero);
	...
	public ReadXMLTestSequence getSecuenciaVictimasClase(
			String rutaFicheroVictimasTest) throws Exception {
		ReadXMLTestSequence rXMLTSeq;
		rXMLTSeq = new ReadXMLTestSequence(rutaFicheroVictimasTest);
		return rXMLTSeq;
	}

	public Document getDocumentVictimas(ReadXMLTestSequence rXMLTSeq)
			throws Exception {
		if (rXMLTSeq != null)
	...
	public NodeList getListaNodosVictima(ReadXMLTestSequence rXMLTSeq,
			Document doc) throws Exception {
		if ((rXMLTSeq != null) && (doc != null))
			return rXMLTSeq.getVictimsXMLStructure(doc, "victim"); // Obtain all
																	// the
																	// victims
		else
			return null;
	...
	private Victim createNewVictim(ReadXMLTestSequence rXMLTSeq,
		...
		// int posicionVictimaNodoLst = numeroVictima - 1; // El nodo de la
		// victima
	...
	public void guardarInfoCasoSimulacion(InfoCasoSimulacion infoCaso)
			throws Exception {
		// se guarda la serie asignacion de victimas. Las series graficas se
		// guardan por orden del agente
	}

	public void guardarInfoAsignacionVictima(
			InfoAsignacionVictima infoAsignVictima) throws Exception {

		infoVictimasAsigandas.add(infoAsignVictima);
		FileOutputStream fInfoAsignVictima = new FileOutputStream(
				identFicheroInfoAsigVictimasObj);
		ObjectOutputStream oPersAsignVictima = new ObjectOutputStream(
				fInfoAsignVictima);
	...
	public ArrayList<InfoAsignacionVictima> obtenerInfoAsignacionVictimas()
			throws Exception {
		// String nombreFicheroAsignVictim = "pruebaAsigVictimas";
		if (infoVictimasAsigandas.isEmpty())
			return null;
		String directorioPersistencia = VocabularioRosace.IdentDirectorioPersistenciaSimulacion
				+ File.separator;
		// String identFichero =
		// directorioPersistencia+nombreFicheroAsignVictim;
		File ficheroPersistencia = new File(identFicheroInfoAsigVictimasObj);
		if (ficheroPersistencia.exists()) {

			FileInputStream fInInfoAsignVictima = new FileInputStream(
					identFicheroInfoAsigVictimasObj);
			ObjectInputStream inPersAsignVictima = new ObjectInputStream(
					fInInfoAsignVictima);
			// ArrayList<InfoAsignacionVictima> infoAsignVictimas = new
			// ArrayList<InfoAsignacionVictima>();
			// infoAsignVictimas =
			return (ArrayList<InfoAsignacionVictima>) inPersAsignVictima
					.readObject();
		} else
			return null;
	}

	public boolean guardarSerieResultadosXML(
			InfoSerieResultadosSimulacion serieResultados) {
		// Se guarda en un fichero xml

		try {
			String identFicheroSerie = directorioPersistencia
					+ serieResultados.getTituloSerie() + ".xml";
			// FileOutputStream fserieResultados = new
			// FileOutputStream(identFicheroSerie);
			Path path = FileSystems.getDefault()
					.getPath(".", identFicheroSerie);
			OutputStream in = Files.newOutputStream(path);
			BufferedWriter escritorXML = Files.newBufferedWriter(FileSystems
					.getDefault().getPath(".", identFicheroSerie), Charset
					.forName("US-ASCII"), StandardOpenOption.CREATE);
			// PrintWriter xmlWriter = new PrintWriter(new
			// FileOutputStream(identFicheroSerie));
			// Writer buffer = new StringWriter();

			XStream xstream = new XStream();
			xstream.alias(serieResultados.getTituloSerie(),
					InfoSerieResultadosSimulacion.class);
	...
	RobotStatus getRobotStatus(String robotId) throws Exception {
		// ReadXMLTestRobots rXMLTRobots = new
		// ReadXMLTestRobots(rutaFicheroRobotsTest);
		// Document doc =
		// rXMLTRobots.getDocument(rXMLTRobots.gettestFilePaht());
		...
		while (j < numeroRobotsSimulation && !encontrado) {
			...
			if (robot.getIdRobot().equals(robotId))
				return robot;
		}
		return null;
	}
	...
	public ArrayList<RobotStatus> getTeamRobotStatus() throws Exception {
		...
		for (int j = 0; j < numeroRobotsSimulation; j++) {
			...
			int energy = rXMLTRobots.getRobotInitialEnergy(info,
					"initialenergy");
