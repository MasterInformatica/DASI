...
public class ClaseGeneradoraRecursoCreacionEntornosSimulacion extends
		ImplRecursoSimple implements ItfUsoRecursoCreacionEntornosSimulacion {
	...
	public ClaseGeneradoraRecursoCreacionEntornosSimulacion(String idRecurso)
			throws Exception {
		...
		try {
			// trazas =
			// (ItfUsoRecursoTrazas)ClaseGeneradoraRepositorioInterfaces.instance()
			...
			notifEvt = new NotificadorInfoUsuarioEdicionEscenarios(idRecurso,
					identAgenteaReportar);
			// un agente debe decirle al recurso a quien debe reportar . Se
			// puede poner el agente a reportar fijo
			controladorIUSimulador = new ControladorEditorEntornosSimulacion(
					notifEvt);
		...
		// " ha completado su ejecucion ....", InfoTraza.NivelTraza.debug));

		// #end_nodeconstructorMethod:constructorMethod <--constructorMethod--
		// DO NOT REMOVE THIS
	}

	// Methods that implement CreacionEntornosSimulacion resource use interface
	@Override
	public void MostrarEscenarioActualSimulado(String identEscenario)
			throws Exception {
	...
	@Override
	public EscenarioSimulacionRobtsVictms getEscenarioRobotsVictims(
			String identEscenario) throws Exception {
		EscenarioSimulacionRobtsVictms escenarioSimulacion = controladorIUSimulador
				.dameEscenario(identEscenario);
		if (escenarioSimulacion == null) {
			String consejo = " El fichero con el escenario solicitado : "
					+ identEscenario + "  No esta definido ";
			String recomendacion = " puede crearlo utilizando el editor ";
			visorSc.visualizarConsejo("Escenario Inexistente", consejo,
					recomendacion);
			if (!editorAbierto)
				controladorIUSimulador.abrirEditor();
			visorSc.setVisible(true);
