...
public class InfoCambioRolAgente extends InfoPropuestaEquipo {
	...
	public InfoCambioRolAgente(String agentId, InfoEquipo equipo,
			String iteracionId) {
		...
		numeroMiembrosEquipoConMiRolInicial = equipo
				.getTeamMemberIDsWithMyRol().size();
		respuestasCambioRol = new EstatusRespuestasEquipo(agtePropietarioId,
				equipo.getTeamMemberIDsWithMyRol(), refIteracion);
	...
	public void setidentRolActualAgte(String agentRolId) {
		if (!identRolActualAgte.equals(agentRolId)) {
			...
			RobotStatus1 miStatus = this.miEquipo
					.getTeamMemberStatus(miAgentId);
			...
			if (identRolAnteriorAgte.equals(miStatus.getIdRobotRol())) {
				...
				NombresPredefinidos.RECURSO_TRAZAS_OBJ
						.trazar(this.agtePropietarioId,
								"Inconsistencia entre los roles definidos y los almacenados antes del Cambio de Rol",
	...
	public synchronized void setequipoAceptaPropuestaCR(
			boolean estatusAceptacion) {
	...
	public void setincicioProcesamientoInformesCR(boolean inicioProcInformesCR) {
		if (!inicicioProcesamientoInformesCR && inicioProcInformesCR) {
			...
			respuestasCambioRol = new EstatusRespuestasEquipo(
					agtePropietarioId, miEquipo.getTeamMemberIDsWithMyRol(),
					refIteracion);
		}
	...
	public void procesarInfoCambioRolAgte(InfoRolAgente infoRol) {
		...
		if (respuestasCambioRol.addRespuestaAgente(identAgteInformante,
				refIteracion)) {
			RobotStatus1 miStatusRobotInformante = this.miEquipo
					.getTeamMemberStatus(identAgteInformante);
			String idRolAgteInformante = infoRol.getidentRolAgte();
			if (!idRolAgteInformante.equals(miStatusRobotInformante
					.getIdRobotRol())) {
	...
	public void verificarCondicionesCambioRolEquipo() {
		// han llegado todas las respuestas esperadas y todos los que tenian el
		// rol antiguo han cambiado al nuevo
