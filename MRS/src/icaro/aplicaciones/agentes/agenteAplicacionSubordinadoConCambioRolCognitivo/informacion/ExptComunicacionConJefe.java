...
public class ExptComunicacionConJefe extends Expectativa {
	...
	public ExptComunicacionConJefe(String agentPropietarioId,
			InfoEquipo equipo, String idJefe) {
		...
		agtesEquipoIds = miEquipo.getTeamMemberIDsWithThisRol(miRol); // los
																		// agtes
																		// que
																		// tiene
																		// el
	...
	public void procesarInfoEstatusComunicacion(
			InfoEstatusComunicacionConOtroAgente comunicEstatus) {
		// Se guarda la informacion recogida del estatus y se verifica si todas
		// las respuestas coinciden en la falta de comunicacion
			...
			for (int i = 0; i < agtesEquipoIds.size(); i++) {
				if (agtesEquipoIds.get(i).equals(identAgteInformante))
					if (!respuestasAgtsEquipo.get(i)) {
						...
						if (!comunicEstatus
								.gethayComunicAgteInformanteConOtroAgte())
							numRespuestasSinConexion++;
					}
			}
		}
	}
	...
	public void validarExpectativa(Object... params) {
		// la expectativa NO se satisface si todos los agentes del equipo
		// confirman la falta de conexion con el jefe
		try {
			...
			trazas.trazar(
					this.getidAgtePropietario(),
					"No se pudo validar la expectativa : "
							+ this.getClass().getSimpleName()
							+ "Verificar el valor de los parametros  utilizados para la verificacion",
