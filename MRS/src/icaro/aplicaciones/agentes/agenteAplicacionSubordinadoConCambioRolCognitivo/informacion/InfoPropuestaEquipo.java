...
public class InfoPropuestaEquipo extends InfoInteraccionBasica {
	...
	public void addAgteAceptaPropuesta(PropuestaAgente aceptPr) {
		if (respuestasEquipo.addRespuestaAgente(aceptPr.identAgente,
				aceptPr.getMensajePropuesta())) {
	...
	public void actualizarInfoPropuesta() {
		if (this.hanLlegadoTodasLasResPuestasDeMiembrosDelEquipo())
			...
			} else if (!propuestaAceptada
					&& getHayConsenso(numMiembrosEquipoParaConsenso))
				propuestaAceptada = true;
	}

	public void procesarRechazarPropuesta(PropuestaAgente rechazoPr) {
		if (respuestasEquipo.addRespuestaAgente(myId,
				rechazoPr.getMensajePropuesta())) {
			agtsEquipoRechazanPropuesta[equipoIds
					.indexOf(rechazoPr.identAgente)] = true;
	...
	public boolean getHayConsenso(int numMiembrosParaAcuerdo) {
		// Puede usarse cuando no han llegado todas las respuestas pero una
		// mayoria ha aceptado una propuesta y no hay rechazos
