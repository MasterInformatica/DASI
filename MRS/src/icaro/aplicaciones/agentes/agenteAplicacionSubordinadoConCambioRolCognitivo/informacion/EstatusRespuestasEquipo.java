...
public class EstatusRespuestasEquipo {
	...
	public EstatusRespuestasEquipo(String agentId, ArrayList<String> equipoIds,
			String iteracionId) {
	...
	public boolean addRespuestaAgente(String identOtroAgte, String iteracionId) {
		if ((respuestasEsperadas > respuestasRecibidas)
				&& (identIteracion.equals(iteracionId))) {
			for (int i = 0; i < agtesEquipoIds.size(); i++) {
	...
	public ArrayList<String> getAgtesFaltanRespuestas(String iteracionId) {
		if (agtesEquipoIds.isEmpty() || !(identIteracion.equals(iteracionId)))
			return null;
		...
		for (int i = 0; i < agtesEquipoIds.size(); i++) {
			if (!respuestasAgtsEquipo.get(i)) {
				...
				if (!idAgteSinRespuesta.equals(idAgtePropietario))
					agtesEquipoSinRespuesta.add(idAgteSinRespuesta);
			}
		}
	...
	public ArrayList<String> getAgtesHanRespondido(String iteracionId) {
		...
		for (int i = 0; i < agtesEquipoIds.size(); i++) {
			if (!respuestasAgtsEquipo.get(i)) {
				idAgteSinRespuesta = agtesEquipoIds.get(i);
				if (!idAgteSinRespuesta.equals(idAgtePropietario))
					agtesEquipoHanRespondido.add(idAgteSinRespuesta);
			}
		}
