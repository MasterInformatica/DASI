...
public class ProcesarPropuestasCambioRol extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			if (propuestaRecibida.getMensajePropuesta().equals(
					VocabularioRosace.MsgPropuesta_CambioRolaIgualitario)
					&& propuestaRecibida.justificacion
							.equals(VocabularioRosace.MsgJustificPetionEstatusComunConJefe)) {
				// si nos propone cambiar de rol y el agte le ha enviado una
				// propuesta similar se considerea la propuesta
				...
				// se modifica el estado de las variables booleanas que se
				// utilizan en las reglas
				/*
				 * if
				 * (mInfoCambioRol.tengoTodasLasRespuestasApropuestaCRreEnviada
				 * ()) if(mInfoCambioRol.getequipoAceptaPropuestaCR())
				 * mInfoCambioRol
				 * .setseVerificanLasCondicionesParaCambiarMirolAigualitario();
				 * else //
				 */
			}
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
					"Se actualiza InfoCambioRol:  " + mInfoCambioRol
							+ "Por procesamiento de una  propuesta recibida : "
							+ propuestaRecibida, InfoTraza.NivelTraza.debug));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
