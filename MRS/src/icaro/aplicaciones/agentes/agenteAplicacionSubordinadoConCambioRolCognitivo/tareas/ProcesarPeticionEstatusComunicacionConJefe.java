...
public class ProcesarPeticionEstatusComunicacionConJefe extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			if (peticionRecibida.mensajePeticion
					.equals(VocabularioRosace.MsgPeticionEnvioEstatusComunicacionConJefe)
					&& peticionRecibida.justificacion
							.equals(VocabularioRosace.MsgJustificPetionEstatusComunConJefe)) {
				// si nos pide el estatus con esa justificacion asumimos que no
				// tiene comunicacion con el jefe
				// se obtiene el nombre del agente que tiene el rol de jefe
				String identAgtePeticionario = peticionRecibida.identAgente;
				comunicStatusRecibido = new InfoEstatusComunicacionConOtroAgente(
						identAgtePeticionario,
						expectativaParaValidar.getidentAgteRolJefe());
				comunicStatusRecibido
						.sethayComunicAgteInformanteConOtroAgte(Boolean.FALSE);
			}
			...
			trazas.aceptaNuevaTraza(new InfoTraza(
					...
					"Se actualiza la expectativa:  "
							+ expectativaParaValidar
							+ "Por procesamiento del InfoEstatusComunicacionConOtroAgente : "
							+ comunicStatusRecibido, InfoTraza.NivelTraza.debug));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
