...
public class MandarMiNuevoRolAlEquipo extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		...
		try {
			trazas.aceptaNuevaTraza(new InfoTraza(miAgentId,
					"Se Ejecuta la Tarea :" + identDeEstaTarea,
					InfoTraza.NivelTraza.debug));
			InfoRolAgente miNuevoRol = new InfoRolAgente(miAgentId,
					miInfoCambioRol.getmiEquipo().getTeamId(),
					miInfoCambioRol.getidentRolActualAgte(),
					VocabularioRosace.IdentIteracionProcesoInformesCR);
			...
			this.getComunicator().informaraGrupoAgentes(miNuevoRol,
					miInfoCambioRol.getmiEquipo().getTeamMemberIDsWithMyRol());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
