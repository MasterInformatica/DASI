...
public class CambiarMiRolAIgualitario extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			// Copio los objetos de la memoria antes de cambiar el
			// comportamiento
			Collection<Object> copiaDeMiMemoria = this.getEnvioHechos()
					.copiarObjetosDeMiMemoria();
			if (this.getEnvioHechos().cambiarComportamiento(
					rutaReglaIgualitario)) {
				// Comprobar que el comportamiento ha cambiado y asertarlo en el
				// motor
				...
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
						"Se cambia el rol del agente jerarquico  por gualitario :  "
								+ rutaReglaIgualitario,
						InfoTraza.NivelTraza.debug));
				// copio los objetos de la memoria antigua a la nueva
				this.getEnvioHechos().insertarObjetosEnMiMemoria(
						copiaDeMiMemoria);
				// actualizo mi robot Status e InfoEquipo con el nuevo rol y lo
				// meto en la memoria
				mInfoCambioRol
						.setidentRolActualAgte(VocabularioRosace.IdentRolAgtesIgualitarios);
