...
public class AccionesSemanticasAgenteAplicacionAgteControladorSimuladorRosace
	...
	// REMOVE THIS
	private ReadXMLTestSequence rXMLTSeq; // para leer del fichero de xml de
											// victimas
	...
	private long tiempoInicialDeLaSimulacion = 0; // Variable para obtener el
													// tiempo al iniciar la
													// simulacion
	...
	private int numeroVictimasDiferentesSimulacion; // Numero de victimas
													...
													// de simulacion
	private int numeroRobotsSimulacion = 0; // Numero de robots diferentes que
											// van a intervenir en el proceso de
											// simulacion
	private int intervaloSecuencia; // Intervalo uniforme de envio de la
									// secuencia de victimas
	private ArrayList<Victim> victimasDefinidas; // Victimas definidas en la
													// simulacion
	private Map<String, Victim> victims2Rescue; // Victimas que han sido
												// enviadas al equipo
	private Map<String, String> victimasAsignadas = new HashMap<String, String>(); // Victimas
																					// que
																					// han
																					// sido
																					// asignadas
	...
	public void AccionComenzar() {
		...
		// ----------------------------------------------------------------------------------------------------------------
		try {
			itfconfig = (ItfUsoConfiguracion) this.itfUsoRepositorio
			...
			itfUsoRecursoVisualizadorEntornosSimulacion
					.setIdentAgenteAReportar(this.nombreAgente);
			itfUsoRecursoVisualizadorEntornosSimulacion
					.setItfUsoPersistenciaSimulador(itfUsoRecursoPersistenciaEntornosSimulacion);
			identFicheroEscenario = itfconfig
					.getValorPropiedadGlobal(VocabularioRosace.NombreFicheroEscenarioSimulacion);
			identificadorEquipo = itfconfig
					.getValorPropiedadGlobal(VocabularioRosace.NombrePropiedadGlobalIdentEquipo);
			equipo = new InfoEquipo(this.nombreAgente, identificadorEquipo);
			identsAgtesEquipo = equipo.getTeamMemberIDs(); // Se obtienen de la
															// configuracion
			comunicator = this.getComunicator();
			this.obtenerEscenarioSimulacion();
			// if (identFicheroEscenario != null){
			// escenarioActual =
			// itfUsoRecursoPersistenciaEntornosSimulacion.obtenerInfoEscenarioSimulacion(identFicheroEscenario);
			//
			// }
			...
			// else{
			// modeloOrganizativo =
			// itfconfig.getValorPropiedadGlobal(VocabularioRosace.NOMBRE_PROPIEDAD_GLOBAL_MODELO_ORGANIZATIVO);
			// this.numeroRobotsSimulacion = identsAgtesEquipo.size();
			// itfUsoRecursoVisualizadorEntornosSimulacion.obtenerEscenarioSimulacion(modeloOrganizativo,
			// numeroRobotsSimulacion);
			// }

			// el primer parametro es una cadena con un caracter en blanco, asi
			// obtengo el equipo correctamente
	...
	public void obtenerEscenarioSimulacion() {
		// Se intenta primero obtenerlo de la descripcion de la orgaizacion y de
		// la persistencia
		// Si no se obtiene nada se le pide a la interfaz de usuario
		try {
			if (identFicheroEscenario != null) {
				escenarioActual = itfUsoRecursoPersistenciaEntornosSimulacion
						.obtenerInfoEscenarioSimulacion(identFicheroEscenario);

			}
			if (escenarioActual != null) {
				escenarioActual.renombrarIdentRobts(identsAgtesEquipo);
				itfUsoRecursoVisualizadorEntornosSimulacion
						.mostrarVentanaControlSimulador(escenarioActual);
				...
				modeloOrganizativo = itfconfig
						.getValorPropiedadGlobal(VocabularioRosace.NOMBRE_PROPIEDAD_GLOBAL_MODELO_ORGANIZATIVO);
				this.numeroRobotsSimulacion = identsAgtesEquipo.size();
				itfUsoRecursoVisualizadorEntornosSimulacion
						.obtenerEscenarioSimulacion(modeloOrganizativo,
								numeroRobotsSimulacion);
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		trazas.trazar(this.nombreAgente,
				"Accion obtenerEscenarioSimulacion completada ....",
				NivelTraza.debug);
	}
	...
	public void ValidarEscenarioRecibido(
		...
		try {
			...
			this.inicializarEstatusRobotsEquipo();
			this.itfUsoRecursoVisualizadorEntornosSimulacion
					.mostrarIdentsEquipoRobots(identsAgtesEquipo);
	...
	// Esta accion semantica se ejecuta cuando se envia el input
	// "sendSequenceOfSimulatedVictimsToRobotTeam" en el
	// metodo sendSequenceOfSimulatedVictimsToRobotTeam de la clase
	// NotificacionEventosRecursoGUI3
	public void sendSequenceOfSimulatedVictimsToRobotTeam(
			Integer intervaloSecuencia) {
		...
		// --------------------------------------------------------------------------------------------------------------------
		// Inicializar y recuperar la referencia al recurso de estadisticas y el
		// visualizador de estadisticas
		// Inicializar el numero de victimas diferentes que hay en la simulacion
		// en el recurso PersistenciaEntornosSimulacion
		// --------------------------------------------------------------------------------------------------------------------
		long tiempoActual = 0;

		try {
			// itfUsoRecursoPersistenciaEntornosSimulacion.setNumeroVictimasDiferentesSimulacion(rXMLTSeq);
			// this.numeroVictimasDiferentesSimulacion =
			// itfUsoRecursoPersistenciaEntornosSimulacion.getNumeroVictimasDiferentesSimulacion();
			// victimasDefinidas =
			// this.itfUsoRecursoPersistenciaEntornosSimulacion.getVictimasArescatar();
			...
			trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
					"Accion SendSequenceOfSimulatedVictimsToRobotTeam  .... "
							...
							+ " ; " + numeroRobotsSimulacion
							+ " robots en la simulacion ; "
							+ numeroVictimasDiferentesSimulacion
							+ " Victimas Diferentes Simulacion "
							+ " (numero Mensajes a Enviar "
							+ this.numMensajesEnviar + ") "
							+ " ; frecuencia de envio " + intervaloSecuencia
							+ " milisegundos", InfoTraza.NivelTraza.debug));
		...
		try {
			...
			// infoContxVict = new
			// InfoContextoAsignacionVictima(identificadorEquipo,
			// numeroVictimasDiferentesSimulacion, numeroRobotsSimulacion,
			// this.intervaloSecuencia);
			infoEntornoCaso = new InfoEntornoCasoSimulacion(
					identificadorEquipo, numeroRobotsSimulacion,
					numeroVictimasDiferentesSimulacion, intervaloSecuencia);
			infoEntornoCaso
					.setTiempoInicioSimulacion(tiempoInicialDeLaSimulacion);
			infoCasoSimul = new InfoCasoSimulacion(identCaso);
			infoCasoSimul.setInfoCasoSimulacion(infoEntornoCaso);
			infoCasoSimul
					.setTiempoInicioEnvioPeticiones(tiempoInicialDeLaSimulacion);
		...
		// ----------------------------------------------------------------------------------------------------------------
		// Crear hilo responsable de realizar el envio de la secuencia de
		// victimas a intervalos regulares de tiempo
		// ----------------------------------------------------------------------------------------------------------------
		...
		Thread t = new Thread() {
			...
			public void run() {
				...
				while ((i < numeroVictimasDiferentesSimulacion)
						&& (stop == false)) {
					// victima = createNewVictim(rXMLTSeq, nodeLst, i);
					victima = (Victim) victimas[i];
					OrdenCentroControl ccOrder = new OrdenCentroControl(
							"ControlCenter",
							VocabularioRosace.MsgOrdenCCAyudarVictima, victima);
					// Escribir nueva linea de estadistica en el fichero de
					// llegada de victimas
					try {
						long tActual = System.currentTimeMillis();

						// Lo siguiente se hacia en
						// escribeEstadisticaFicheroXMLTRealLlegadaVictimas //EN
						// EL FUTURO HABRIA QUE INTENTAR QUITARLO DE ESE METODO
						Victim valor = victims2Rescue.put(victima.getName(),
								victima);
						...
						// itfUsoRecursoPersistenciaEntornosSimulacion.escribeEstadisticaFicheroXMLTRealLlegadaVictimas(tActual,
						// victima);
						// Anotar informacion en el buffer de estadisticas de
						// victimas que llegan al entorno
						InfoAsignacionVictima infoAsigVictima = new InfoAsignacionVictima();
						infoAsigVictima
								.setNrovictimasenentorno(numeroVictimasEntorno);
						infoAsigVictima.setTiempoPeticion(tActual
								- tiempoInicialDeLaSimulacion);
						infoAsigVictima.setVictima(victima);
						infoCasoSimul.addEnvioVictima(infoAsigVictima);
						// /////
						// VictimaLlegadaEstadistica victLlegEst = new
						// VictimaLlegadaEstadistica();
						// victLlegEst.setVictima(victima);
						// victLlegEst.setTiempoActual(tActual);
						// victLlegEst.setNrovictimas(numeroVictimasEntorno);
						// victimasLlegadaEstadistica.add(i-1, victLlegEst);
						// //el buffer empieza en la posicion 0
					...
					{
						comunicator.enviarInfoAotroAgente(ccOrder,
								VocabularioRosace.IdentAgteDistribuidorTareas);
					} else {
				...
				// Cerrar el fichero de estadistica en el fichero de llegada de
				// victimas
	...
	public void sendSimulatedVictimToRobotTeam(String idVictima) {
		trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
				"Accion SendVictimToRobotTeam  .... " + idVictima,
				InfoTraza.NivelTraza.debug));

		if (victims2Rescue == null)
			victims2Rescue = escenarioActual.getVictims();
		Victim victima = victims2Rescue.get(idVictima);
		OrdenCentroControl ccOrder = new OrdenCentroControl("ControlCenter",
				VocabularioRosace.MsgOrdenCCAyudarVictima, victima);
		comunicator.informaraGrupoAgentes(ccOrder, identsAgtesEquipo);
	}

	// Esta accion semantica se ejecuta cuando se envia el input
	// "victimaAsignadaARobot" en la
	// tarea sincrona GeneraryEncolarObjetivoActualizarFoco del agente
	// Subordinado
	// Esta accion semantica se ejecuta cuando se envia el input
	// "victimaAsignadaARobot" en la
	// tarea sincrona EncolarObjetivoActualizarFoco del agente Igualitario
	// (robotMasterIA)
	public void VictimaAsignadaARobot(Long tiempoReportado, String refVictima,
			String nombreAgenteEmisor, Integer miEvaluacion) {
		try {
			trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
					"Accion VictimaAsignadaARobot  ... " + "tiempoActual->"
							+ tiempoReportado + " ; refVictima->" + refVictima
							+ " ; nombreAgenteEmisor->" + nombreAgenteEmisor
							+ " ; miEvaluacion->" + miEvaluacion,
					InfoTraza.NivelTraza.debug));
			itfUsoRecursoVisualizadorEntornosSimulacion
					.mostrarVictimaRescatada(refVictima);
			if (infoCasoSimul != null) {
				InfoAsignacionVictima infoAsigVictima = infoCasoSimul
						.getInfoAsignacionVictima(refVictima);
				infoAsigVictima.setEvaluacion(miEvaluacion);
				infoAsigVictima.setRobotId(nombreAgenteEmisor);
				infoAsigVictima.setTiempoAsignacion(tiempoReportado
						- tiempoInicialDeLaSimulacion); // tiempo reportado
				infoAsigVictima.setNrovictimastotalasignadas(infoCasoSimul
						.getnumeroVictimasAsignadas());
				infoAsigVictima.setNrovictimasenentorno(infoCasoSimul
						.getnumeroVictimasEntorno());
				itfUsoRecursoPersistenciaEntornosSimulacion
						.guardarInfoAsignacionVictima(infoAsigVictima);
				itfUsoRecursoVisualizadorEntornosSimulacion
						.mostrarVictimaRescatada(refVictima);
			...
			Logger.getLogger(
					AccionesSemanticasAgenteAplicacionAgteControladorSimuladorRosace.class
	...
	private void visualizarYguardarResultadosCaso() {
		try {
			...
			itfUsoRecursoVisualizadorEntornosSimulacion
					.crearVisorGraficasLlegadaYasignacionVictimas(
							this.numeroRobotsSimulacion,
							this.numeroVictimasDiferentesSimulacion,
							this.intervaloSecuencia, this.identificadorEquipo); // parametros
																				// definicion
																				// titulos
			ArrayList<PuntoEstadistica> llegada = infoCasoSimul
					.getSerieLlegadaPeticiones().getserieResultadosSimulacion();
			ArrayList<PuntoEstadistica> asignacion = infoCasoSimul
					.getSerieAsignacion().getserieResultadosSimulacion();
			itfUsoRecursoVisualizadorEntornosSimulacion
					.visualizarLlegadaYasignacionVictimas(llegada, asignacion);
			itfUsoRecursoVisualizadorEntornosSimulacion
					.crearVisorGraficasTiempoAsignacionVictimas(
							this.numeroRobotsSimulacion,
							this.numeroVictimasDiferentesSimulacion,
							this.intervaloSecuencia, this.identificadorEquipo); // parametros
																				// definicion
																				// titulos
			ArrayList<PuntoEstadistica> elapsed = infoCasoSimul
					.getSerieElapsed().getserieResultadosSimulacion();
			itfUsoRecursoVisualizadorEntornosSimulacion
					.visualizarTiempoAsignacionVictimas(elapsed);
			itfUsoRecursoPersistenciaEntornosSimulacion
					.guardarSerieResultadosSimulacion(infoCasoSimul
							.getSerieAsignacion());
			itfUsoRecursoPersistenciaEntornosSimulacion
					.guardarSerieResultadosSimulacion(infoCasoSimul
							.getSerieElapsed());
	...
	public void procesarInfoAsignacionVictima(Long tiempoAsignacion,
			String refVictima, String nombreAgenteEmisor, Integer miEvaluacion) {
		// el robot que se ha quedado con la victima informa sobre los detalles
		// de la asingnacion
		// este agente incorpora el contexto de asigancion de la victima
		trazas.trazar(this.nombreAgente, "Info Asigancion Victima  ... "
				+ "tiempoActual->" + tiempoAsignacion + " ; refVictima->"
				+ refVictima + " ; nombreAgenteEmisor->" + nombreAgenteEmisor
				+ " ; miEvaluacion->" + miEvaluacion,
				InfoTraza.NivelTraza.debug);
		InfoAsignacionVictima infoAsigVictima = infoCasoSimul
				.getInfoAsignacionVictima(refVictima);
		...
		infoCasoSimul.addAsignacionVictima(infoAsigVictima);
		if (infoCasoSimul.todasLasVictimasAsgnadas()) {
			this.informaraMiAutomata(
					VocabularioRosace.informacionFinSimulacion, null);
		}

	}

	// Esta accion semantica se ejecuta cuando se envia el input "finSimulacion"
	// en la
	// tarea sincrona FinalizarSimulacion del agente Subordinado y el
	// igualitario
	// Nos permite generar un fichero
	// EstadisticaFinalSimulacionAsignacionMisionV2.xml que resume que victimas
	// han sido asignadas a cada robot.

	public void FinSimulacion(String robot,
			ArrayList idsVictimasFinalesAsignadas,
			Double tiempoTotalCompletarMisionAtenderVictimasFinalesAsignadas) {

		trazas.aceptaNuevaTraza(new InfoTraza(
				this.nombreAgente,
				"Accion FinSimulacion  .... "
						+ "robot->"
						+ robot
		...
		try {
			...
			infoAsignVictms = itfUsoRecursoPersistenciaEntornosSimulacion
					.obtenerInfoAsignacionVictimas();
			contadorRobotsQueContestanFinsimulacion++;
			if (contadorRobotsQueContestanFinsimulacion == identsAgtesEquipo
					.size())
				this.itfUsoRecursoVisualizadorEntornosSimulacion
						.mostrarResultadosFinSimulacion();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void crearTempSolicitarDefinicionEscenario() {
		...
		this.generarTimeOutInterno(tiempoEsperaDefinicionEscenario,
				"timeoutEsperaDefinicionEscenario", nombreAgente,
	...
	public void solicitarDefinicionEscenario() {
		try {
			itfUsoRecursoVisualizadorEntornosSimulacion
					.notificarRecomendacion(
							"Sin escenario Definido",
							"No se puede iniciar la simulacion sin definir un escenario",
	...
	public void mostrarEscenarioActualSimulado() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
				"Accion MostrarEscenarioActualSimulado  ....",
				InfoTraza.NivelTraza.debug));
		try {
	...
	public void mostrarRobotsActivos() {
		try {
			itfUsoRecursoVisualizadorEntornosSimulacion
					.mostrarIdentsEquipoRobots(this.identsAgtesEquipo);
		} catch (Exception ex) {
			Logger.getLogger(
					AccionesSemanticasAgenteAplicacionAgteControladorSimuladorRosace.class
							.getName()).log(Level.SEVERE, null, ex);
	...
	public void inicializarEstatusRobotsEquipo() {
		// se envia a cada robot su estatus particular en el escenario :
		// situacion, energia, rol y demas
		String identRobot;
		RobotStatus1 estatusRobot;
		for (Object identsAgtesEquipo1 : identsAgtesEquipo) {
			identRobot = (String) identsAgtesEquipo1;
			estatusRobot = (RobotStatus1) escenarioActual.getRobotInfo(
					identRobot).clone();
			// RobotStatus estatusRobotCopia =
			// (RobotStatus)estatusRobot.clone();
	...
	private void informarResultadosSimulacion() {
		try {
			...
			this.itfUsoRecursoVisualizadorEntornosSimulacion
					.visualizarLlegadaYasignacionVictimas(identsAgtesEquipo,
							identsAgtesEquipo);
			this.itfUsoRecursoVisualizadorEntornosSimulacion
					.visualizarTiempoAsignacionVictimas(identsAgtesEquipo);
