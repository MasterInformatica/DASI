		...
		try {
			...
			if (!infoDecision.peticionEvaluacionEnviadaAtodos) {
				...
				if (numAgtesEnEquipo == 0)
					...
					peticionEval.setJustificacion(victima); // para que se sepa
															// qué evaluacion le
															// pedimos
					this.getComunicator().informaraGrupoAgentes(peticionEval,
							agentesEquipo);
					infoDecision.setRespuestasEsperadas(agentesEquipo.size());
					infoDecision
							.setPeticionEvaluacionEnviadaAtodos(Boolean.TRUE);
					...
					// Interesa dar un nombre especifico a la tarea que genera
					// la temporizacion. El informe debe ser tambien especifico
					// se pone el identificador de la victima
