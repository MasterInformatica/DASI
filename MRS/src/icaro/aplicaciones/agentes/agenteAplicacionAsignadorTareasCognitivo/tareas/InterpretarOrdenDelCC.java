	...
	public void ejecutar(Object... params) {
		try {
			...
			this.getEnvioHechos().eliminarHechoWithoutFireRules(
					ccOrdenAyudarVictima);
			// verificamos que no se esta ayudando a esa victima. Comprobamos
			// que el ident no esta en ninguno de los objetivos
			// if
			// ((ccOrdenAyudarVictima.mensajeOrden.equals(VocabularioRosace.MsgOrdenCCAyudarVictima)))
			// if((objetivoEjecutantedeTarea == null) |
			// ((objetivoEjecutantedeTarea !=
			// null)&&((!idVictim.equals(objetivoEjecutantedeTarea.getobjectReferenceId()))&&
			// (!misObjs.existeObjetivoConEsteIdentRef(idVictim))))){
			// se crea el objetivo y se inserta en el motor
			if (victims2R.getvictims2Rescue().isEmpty()
					|| victims2R.getVictimToRescue(idVictim) == null) {
				...
				// if((objetivoEjecutantedeTarea == null))
				// newObjetivo.setSolving(); // se comienza el proceso para
				...
				this.getEnvioHechos().insertarHechoWithoutFireRules(
						newAyudarVictima);
				this.getEnvioHechos()
						.actualizarHechoWithoutFireRules(victims2R);
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						"\n" + nombreAgenteEmisor, "Se ejecuta la tarea "
								+ this.getIdentTarea()
				...
				// ((ccOrdenAyudarVictima.mensajeOrden.equals(VocabularioRosace.MsgOrdenCCAyudarVictima))
				// &&
				// (!idVictim.equals(objetivoEjecutantedeTarea.getobjectReferenceId()))&&
				// (!misObjs.existeObjetivoConEsteIdentRef(idVictim)) ){
