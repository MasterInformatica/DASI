...
public class InfoParaDecidirAQuienAsignarObjetivo implements Serializable {
	...
	public int numeroEvaluacionesRecibidas, valorMejorEvalucionRecibida,
	...
	public InfoParaDecidirAQuienAsignarObjetivo(String identAgente,
			InfoEquipo equipo) {
		try {
			// trazas = NombresPredefinidos.RECURSO_TRAZAS_OBJ;

			// ItfUsoConfiguracion itfConfig = (ItfUsoConfiguracion)
			// NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ.obtenerInterfaz(NombresPredefinidos.ITF_USO+"Configuracion");
			// agentesAplicacionDefinidos =
			// itfConfig.getIdentificadoresInstanciasAgentesAplicacion();
			// agentesAplicacionDefinidos =
			// equipo.getIdentificadoresInstanciasAgentesAplicacion();
			// identEquipo =
			// itfConfig.getValorPropiedadGlobal(NombresPredefinidos.NOMBRE_PROPIEDAD_GLOBAL_EQUIPO_AGENTES);
			...
			Logger.getLogger(
					InfoParaDecidirAQuienAsignarObjetivo.class.getName()).log(
			...
			if (identEquipo != null) {
				...
				for (int i = 0; i < agentesAplicacionDefinidos.size(); i++) {
					...
					if (!aux.contains(identAgente))
						// esto hay que quitarlo y nombrar los agentes como es
						// debido *********
						if (identEquipo
								.equals(VocabularioRosace.IdentEquipoJerarquico))
							agentesEquipo.add(aux);
						else if (aux.contains(identEquipo)
								&& !aux.contains(identAgente)) {
							agentesEquipo.add(aux);
						}
				}
	...
	public String toString() {
		return " idElementoDecision->" + idElementoDecision
				...
				+ " ; tengoLaMejorEvaluacion->"
				+ tengoLaMejorEvaluacion
				+ " ; hePedidoEvaluacionAtodos->"
				+ peticionEvaluacionEnviadaAtodos
				+ " ; hayEmpates->"
				+ hayEmpates
				+
