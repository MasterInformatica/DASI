...
public class PruebaCambioRol extends Tarea {
	...
	public void ejecutar(Object... params) {
		try {
			...
			/*
			 * InputStream reglas =
			 * this.getClass().getResourceAsStream(rutaNuevasReglas);
			 * this.getItfMotorDeReglas().crearKbSesionConNuevasReglas(reglas,
			 * rutaNuevasReglas);
			 * this.getItfMotorDeReglas().addGlobalVariable(NombresPredefinidos
			 * .TASK_MANAGER_GLOBAL,
			 * this.getEnvioHechos().GetItfGestorTareas());
			 * this.getItfMotorDeReglas
			 * ().addGlobalVariable(NombresPredefinidos.ITFUSO_RECURSOTRAZAS_GLOBAL
			 * , trazas);
			 * this.getItfMotorDeReglas().addGlobalVariable(NombresPredefinidos
			 * .AGENT_ID_GLOBAL, nombreAgenteEmisor);
			 */
			if (this.getEnvioHechos().cambiarComportamiento(rutaNuevasReglas))
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
						"Se cambia el rol del agente jerarquico  por gualitario :  "
								+ rutaNuevasReglas, InfoTraza.NivelTraza.debug));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
