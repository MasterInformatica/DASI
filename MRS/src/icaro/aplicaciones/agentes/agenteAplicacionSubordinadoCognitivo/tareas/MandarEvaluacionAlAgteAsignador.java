...
public class MandarEvaluacionAlAgteAsignador extends TareaSincrona {
	...
	private ArrayList agentesEquipo, respuestasAgentes, confirmacionesAgentes,
			nuevasEvaluacionesAgentes, empates;// resto de agentes que forman mi
												// equipo
	...
	public void ejecutar(Object... params) {
		...
		// nombreAgenteEmisor);
		// agentesEquipo =
		// infoDecision.getNombreAgentesEquipoDefinidos(nombreAgenteEmisor,
		// VocabularioRosace.IdentComunAgtesSubordinados);
		...
		try {
			...
			// si el identificador esta entre mis objetivos es que esta resuelto
			// , le mando un valor para que se desanime
			// en otro caso puede ser que sea otro el que tenga el objetivo, en
			// este caso él le mandará un valor grande
			// si no tengo noticias del objetivo le mando un valor pequeno para
			// que vaya él
			// si coincide con el que estoy trabajando le mando el valor
			// if (identObjEvaluacion.equals(infoDecision.idElementoDecision))
			// miEvalDeRespuesta = infoDecision.getMi_eval();
			// else
			if (misObjtvs.existeObjetivoConEsteIdentRef(identObjEvaluacion))
				...
				victimEnPeticion = victimasRecibidas
						.getVictimToRescue(identObjEvaluacion);
				if (victimEnPeticion != null) { // tengo la victima miro si
												// tengo el valor estimado
					if (victimEnPeticion.isCostEstimated())
				...
				} else {// la victima no existe -> no se ha recibido el mensaje
						// del CC. Calculo el coste y meto los objetivos y la
						// victima
					victimEnPeticion = (Victim) peticionRecibida
							.getJustificacion();
					miEvalDeRespuesta = calcularCosteEstimadoVictima();
					victimEnPeticion.setEstimatedCost(miEvalDeRespuesta);
					// AyudarVictima newAyudarVictima = new AyudarVictima
					// (identObjEvaluacion);
					// newAyudarVictima.setPriority(victimEnPeticion.getPriority());
					victimasRecibidas.addVictimToRescue(victimEnPeticion);
					// DecidirQuienVa newDecision = new
					// DecidirQuienVa(identObjEvaluacion);
			...
			EvaluacionAgente miEvaluacion = new EvaluacionAgente(
					nombreAgenteEmisor, miEvalDeRespuesta);
			miEvaluacion.setObjectEvaluationId(identObjEvaluacion);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
					"Enviamos la evaluacion " + miEvaluacion,
					InfoTraza.NivelTraza.info));

			this.getComunicator().enviarInfoAotroAgente(miEvaluacion,
					nombreAgenteQuePideLaEvaluacion);
			this.generarInformeOK(
					identTarea,
					objetivoEjecutantedeTarea,
					nombreAgenteEmisor,
			...
			// InfoTraza.NivelTraza.info));
			trazas.aceptaNuevaTrazaEjecReglas(nombreAgenteEmisor,
					"Victima En Peticion : " + victimEnPeticion
							+ "  EvaluacionEnviadaAlAgente : "
							+ nombreAgenteQuePideLaEvaluacion
							+ " Evaluacion : " + miEvalDeRespuesta);
	...
	private int calcularCosteEstimadoVictima() {
		 ...
		 * .obtenerInterfaz(NombresPredefinidos.ITF_USO + "RecursoMorse1");
		...
		return coste.CalculoCosteAyudarVictima(nombreAgenteEmisor,
				robotLocation, robot, victimEnPeticion, victimasRecibidas,
				misObjtvs, "FuncionEvaluacion3");
