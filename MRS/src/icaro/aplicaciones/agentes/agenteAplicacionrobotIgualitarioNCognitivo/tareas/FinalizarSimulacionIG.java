...
public class FinalizarSimulacionIG extends TareaSincrona {
	...
	private ItfUsoRecursoEstadistica itfUsoRecursoEstadistica; // Para escribir
																// estadisticas
	...
	public void ejecutar(Object... params) {
		try {
			...
			// trazas.aceptaNuevaTraza(new InfoTraza("Evaluacionadfd", " ",
			// InfoTraza.NivelTraza.info));
			// trazas.aceptaNuevaTraza(new InfoTraza("Evaluacionadfd",
			// nombreAgenteEmisor + " ejecutando " + identTarea,
			// InfoTraza.NivelTraza.info));
			...
			// Las sentencias siguientes permiten calcular el tiempo en realizar
			// la mision, es decir, atender a las victimas de las que se ha
			// hecho responsable
			// utilizar la funcion de evaluacion 3 que considera el recorrido
			// que tendria que hacer y el tiempo en curar cada victima

			double tiempoRecorrerCaminoVictimasAsignadas = coste
					.calculaCosteRecorrerCaminoVictimas(nombreAgenteEmisor,
							robotLocation, victims2R, misObjs);
			double tiempoAtencionVictimasAsignadas = coste
					.calculaCosteAtencionVictimasFinalesAsignadas(3.0,
							victims2R, misObjs);
			double tiempoTotalCompletarMisionAtenderVictimasFinalesAsignadas = coste
					.calculaCosteTotalCompletarMisionAtenderVictimasFinalesAsignadas(
							tiempoRecorrerCaminoVictimasAsignadas, 10.0,
							tiempoAtencionVictimasAsignadas, 3.0);

			// Obtener las victimas que tiene asignadas el robot
			this.idsVictimasFinalesAsignadas = getIdsVictimasFinalesAsignadas(
					misObjs, victims2R);

			// Crear los ficheros EstadisticaFinalSimulacionAsignacionMision
			// (.xml y .txt)
			...
			itfUsoRecursoEstadistica
					.crearFicherosRepartoTareasRobotsYTiempoCompletarlas(
							filenameXMLV1, filenameXMLV2, filenameTXT);

			// Escribir estadisticas en los ficheros creados previamente
			itfUsoRecursoEstadistica
					.escribeEstadisticaFicherosRepartoTareasRobotsYTiempoCompletarlas(
							nombreAgenteEmisor,
			...
			System.out.println(nombreAgenteEmisor
					+ " : tiempo para recorrer camino victimas asignadas->"
					+ tiempoRecorrerCaminoVictimasAsignadas);
			System.out.println(nombreAgenteEmisor
					+ " : tiempo para atender victimas asignadas->"
					+ tiempoAtencionVictimasAsignadas);
			System.out
					.println(nombreAgenteEmisor
							+ " : tiempo total mision ->"
							+ tiempoTotalCompletarMisionAtenderVictimasFinalesAsignadas);
	...
	private ArrayList getIdsVictimasFinalesAsignadas(MisObjetivos misObjs,
			VictimsToRescue victims2R) {
