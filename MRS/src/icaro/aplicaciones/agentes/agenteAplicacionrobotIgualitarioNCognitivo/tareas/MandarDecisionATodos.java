...
public class MandarDecisionATodos extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			DecisionAgente decision = new DecisionAgente(nombreAgenteEmisor,
					msgDecision);
			decision.setidentObjectRefDecision(identObjetoDecision); // En este
																		// caso
																		// el
																		// identificador
																		// se
			...
			trazas.aceptaNuevaTraza(new InfoTraza("OrdenAsignacion",
					"El robot " + this.identAgente
							+ " se hace cargo de la victima "
							+ identObjetoDecision, InfoTraza.NivelTraza.debug));
			this.getComunicator().informaraGrupoAgentes(decision,
					infoDecision.getAgentesEquipo());
			infoDecision
					.setMiDecisionParaAsumirElObjetivoEnviadaAtodos(Boolean.TRUE);
			// se prodria quitar
			...
			this.generarInformeOK(identTarea, objetivoEjecutantedeTarea,
					nombreAgenteEmisor, "DecisionDeIrEnviadaAtodos");
			// trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
