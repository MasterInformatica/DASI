	...
	public void ejecutar(Object... params) {
		try {
			...
			if (infoDecision.hayEmpates) {
				...
				DecisionAgente decision = new DecisionAgente(
						nombreAgenteEmisor,
						VocabularioRosace.MsgPropuesta_Decision_Ir);
				decision.setidentObjectRefDecision(infoDecision
						.getidElementoDecision()); // En este caso el
													// identificador se refiere
													// a la victima
				decision.setJustificacion(infoDecision.getMi_eval());

				this.getComunicator().informaraGrupoAgentes(decision,
						infoDecision.getAgentesEquipo());
				this.generarInformeOK(identDeEstaTarea,
						objetivoEjecutantedeTarea, nombreAgenteEmisor,
						"DecisionDeIrEnviadaAtodos");
				// infoDecision.hayEmpates = false;
				// infoDecision.tengoLaMejorEvaluacion = true;
				infoDecision
						.setMiDecisionParaAsumirElObjetivoEnviadaAtodos(true);
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
						"Enviamos la decision: " + decision,
