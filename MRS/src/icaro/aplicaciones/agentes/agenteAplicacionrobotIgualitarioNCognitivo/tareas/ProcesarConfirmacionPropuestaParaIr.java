...
public class ProcesarConfirmacionPropuestaParaIr extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			PropuestaAgente propuestaConfirmada = confirmacionRecibida
					.getpropuestaAceptada();
			String msgPropuestaOriginal = propuestaConfirmada
					.getMensajePropuesta();
			// if
			// ((msgPropuestaOriginal.equals(VocabularioRosace.MsgPropuesta_Para_Q_vayaYo)
			// )||(msgPropuestaOriginal.equals(
			...
			if ((msgPropuestaOriginal != VocabularioRosace.MsgPropuesta_Decision_Ir)
				...
				this.generarInformeConCausaTerminacion(identTarea,
						...
						"LaPropuestaConfirmadaNoEsValida",
						CausaTerminacionTarea.ERROR);
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgenteEmisor,
						"La propuesta Confirmada No es valida :"
								+ propuestaConfirmada,
						InfoTraza.NivelTraza.error));
			} else {
				// JM: El contenido de la confirmacion de propuestaaceptada era
				// YoVoy o SoyElMejorSituadoParaRealizarElObjetivo
				// la siguiente llamada actualiza la variable
				// tengoAcuerdoDeTodos de InfoParaDecidirQuienVa
				// se volvera a meter al motor
				infoDecisionAgente.addConfirmacionAcuerdoParaIr(
						nombreAgenteEmisorRespuesta,
				...
				if (infoDecisionAgente.getTengoAcuerdoDeTodos()) {
					if (infoDecisionAgente.tengoLaMejorEval()) {
						this.generarInformeOK(identTarea,
								objetivoEjecutantedeTarea, nombreAgenteEmisor,
			...
			trazas.aceptaNuevaTraza(new InfoTraza(
					nombreAgenteEmisor,
					"El numero de respuestas confirmadas para irYo es  :"
							+ infoDecisionAgente.numeroRespuestasConfirmacionParaIrYo,
					InfoTraza.NivelTraza.info));
