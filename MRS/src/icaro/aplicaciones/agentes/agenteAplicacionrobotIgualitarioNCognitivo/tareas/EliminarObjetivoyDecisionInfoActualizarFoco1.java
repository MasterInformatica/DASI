 ...
 * @author FGarijo
 * 
 *         Corresponde a un decisi�n de que otro agente se ha hecho cargo de la
 *         tarea
 */
public class EliminarObjetivoyDecisionInfoActualizarFoco1 extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			Objetivo ayudarVictima = (Objetivo) params[1]; // el estado es
															// pending
			InfoParaDecidirQuienVa infoDecision = (InfoParaDecidirQuienVa) params[2];
			Focus focoActual = (Focus) params[3]; // el foco actual es decidir
													// quien va
			// Objetivo objetivoMasPrioritario =
			...
			// }else
			// if(objetivoMasPrioritario.getPriority()<0)objetivoMasPrioritario=null;
			...
			trazas.aceptaNuevaTrazaEjecReglas(this.identAgente,
					"Se ejecuta la tarea " + this.identTarea
							...
							+ "\n");
			System.out.println("\n" + this.identAgente + "Se ejecuta la tarea "
					+ this.getIdentTarea() + " Se actualiza el  objetivo:  "
					+ ayudarVictima + "\n\n");
