...
public class InfoParaDecidirQuienVa implements Serializable {
	...
	private ArrayList confirmacionesAgentes, evaluacionesRecibidas;// resto de
																	// agentes
																	// que
																	// forman mi
																	// equipo
	...
	private ArrayList<String> agentesAplicacionDefinidos, agentesEquipo,
			agentesEmpatados, respuestasAgentes;
	private int mi_eval_nueva, respuestasEsperadas, confirmacionesEsperadas;
	private int numeroEvaluacionesRecibidas, valorMejorEvalucionRecibida,
			indiceAgenteConMejorEvaluacion;
	...
	public boolean noSoyElMejor = true; // Not (hayEmpates) or Not (
										// tengoLaMejorEvaluacion)
	...
	public InfoParaDecidirQuienVa(String identAgente) {
		try {
			...
			ItfUsoConfiguracion itfConfig = (ItfUsoConfiguracion) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ "Configuracion");
			agentesAplicacionDefinidos = itfConfig
					.getIdentificadoresInstanciasAgentesAplicacion();
			identEquipo = itfConfig
					.getValorPropiedadGlobal(NombresPredefinidos.NOMBRE_PROPIEDAD_GLOBAL_IDENT_EQUIPO);
			agentesEquipo = getNombreAgentesEquipoDefinidos(identAgente,
					identEquipo);
			...
			// Inicializamos para cada agente las respuestas, empates,
			// confirmaciones
			String aux;
			...
			Logger.getLogger(InfoParaDecidirQuienVa.class.getName()).log(
	...
	public synchronized ArrayList ObtenerNombreAgentesDelEquipoRegistrados(
			String identEquipo, String identAgente) {
		try {
			ArrayList agentesRegistrados = NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
					.nombresInterfacesRegistradas();

			agentesEquipo = new ArrayList();
			String aux;
			for (int i = 0; i < agentesRegistrados.size(); i++) {
				aux = (String) agentesRegistrados.get(i);
				// filtramos los Gestores. Mandamos solo a las itf de uso de los
				// agentes, y excluimos el propio agente
				if (aux.contains(identEquipo) && aux.contains("Itf_Uso")
						&& !aux.contains(identAgente)) {
	...
	private ArrayList getNombreAgentesEquipoDefinidos(String identAgente,
			String identEquipo) {
		try {
			if (identEquipo != null) {
			...
			// se deberia poner un mensaje de error en las trazas, pero ya
			// saldra mas adelante
		} catch (Exception ex) {
			Logger.getLogger(InfoParaDecidirQuienVa.class.getName()).log(
					Level.SEVERE, null, ex);
	...
	public synchronized boolean tengoLaMejorEval() {
		/*
		 * boolean soyElMejor = true; // mi_eval = miEvaluacion.getEvaluacion();
		 * for(int i = 0; i< evaluacionesRecibidas.size();i++){
		 * if(mi_eval<((Integer)evaluacionesRecibidas.get(i))){ soyElMejor =
		 * false; }
		 * 
		 * 
	...
	public synchronized boolean tengoTodasLasEvaluaciones() {
		/*
		 * boolean todasLasEvaluaciones = true; int i = 0; while
		 * (todasLasEvaluaciones & ( i< agentesEquipo.size())){
		 * if((Integer)evaluacionesRecibidas.get(i)== 0){ todasLasEvaluaciones =
		 * false; } i++; }
		 */
	...
	public synchronized void procesarValorPropuestaDesempate(
		...
		if (hayEmpates) { // si no hay empates no se hace nada
			...
			if (msgPropuesta
					.equals(VocabularioRosace.MsgPropuesta_Oferta_Para_Ir)) { // otro
																				// agente
																				// se
																				// propone
																				// para
				...
				hayEmpates = false;
				noSoyElMejor = true;
			} else if (msgPropuesta
					.equals(VocabularioRosace.MsgPropuesta_Para_Q_vayaYo)) {// otro
																			// agente
																			// dice
																			// que
																			// me
				...
				if (mi_eval < valorPropuesta) {
					noSoyElMejor = true;
					hayEmpates = false; // dejo de estar empatado porque hay
										// otro con mejor evaluacion
					tengoLaMejorEvaluacion = false;
				} else if (mi_eval > valorPropuesta) { // quito al agente de la
														// lista de empatados
					// agentesEmpatados.remove(identAgteProponente); anado la
					// evaluacion a las evaluaciones recibidas
					// EvaluacionAgente evalDelaPropuesta = new
					// EvaluacionAgente(identAgteProponente,valorPropuesta);
					Integer indiceAgente = agentesEquipo
							.indexOf(identAgteProponente);
					evaluacionesRecibidas.set(indiceAgente, valorPropuesta);
					if (this.propuestasDesempateRecibidas == this.propuestasDesempateEsperadas) {
						hayEmpates = false; // dejo de estar empatado porque
											// tengo la mejor evaluacion
						tengoLaMejorEvaluacion = true;
					}
				}
			}
	...
	public synchronized void procesarPropuestaRecibida(PropuestaAgente propuesta) {
		...
		// "Respuesta Recibida. Dice :" +
		// eval.toString()+" Emisor: "+idAgenteEmisorEvaluacion,
		// InfoTraza.NivelTraza.info));
		// actualizar las respuestas
		Integer indiceAgenteEmisorRespuesta = agentesEquipo
				.indexOf(idAgenteEmisorRespuesta);

		if ((String) respuestasAgentes.get(indiceAgenteEmisorRespuesta) == "") {
			respuestasRecibidas++;

			respuestasAgentes.set(indiceAgenteEmisorRespuesta, respuesta);// guardamos
																			// la
																			// respuesta
			if ((respuesta == "CreoQueDebesIrTu") & (tengoLaMejorEvaluacion)) {
				// añado la respuesta a la lista de empates Puedo comprobar su
				// evaluacion pero me fio de el
				// empates.add(idAgenteEmisorRespuesta);//quiere decir que el
				// agente de agentesequipo(i) esta empatado conmigo
				// hayOtrosQueQuierenIr = true;
				numeroRespuestasConfirmacionParaIrYo++;
				this.addConfirmacionAcuerdoParaIr(idAgenteEmisorRespuesta,
						respuesta);
	...
	public synchronized void addConfirmacionAcuerdoParaIr(
			String idAgenteEmisorRespuesta, String respuesta) {
		...
		if (((String) confirmacionesAgentes.get(indiceAgenteEmisorRespuesta))
				.equals("")) {
			numeroRespuestasConfirmacionParaIrYo++;
			confirmacionesAgentes.set(indiceAgenteEmisorRespuesta, respuesta);// guardamos
																				// la
																				// respuesta
	...
	public synchronized void addNuevaEvaluacion(EvaluacionAgente evaluacion) {
		...
		Integer indiceAgente = agentesEquipo.indexOf(evaluacion
				.getIdentAgente());
		// Si el agente no pertenece al equipo ignoro la evalucion que puede ser
		// la mia
		if (indiceAgente != -1) {
			// trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
			// "Respuesta Recibida. Dice :" +
			...
			if (eval > valorMejorEvalucionRecibida) {
				// Si la evaluacion recibida es mayor que la mejor evaluacion
				// actualizo el valor de la mejor evaluacion
			...
			if ((Integer) evaluacionesRecibidas.get(indiceAgente) == 0) {
				// Si recibimos otra evaluacion del mismo agente no
				// incrementamos
				numeroEvaluacionesRecibidas++;
			}

			evaluacionesRecibidas.set(indiceAgente, eval);// guardamos la
															// evaluacion
															// recibida

			if (numeroEvaluacionesRecibidas == agentesEquipo.size()) {
				hanLlegadoTodasLasEvaluaciones = true;
				// Caluculo si tengo la mejor evaluacio o si hay empate con
				// otros
				if (mi_eval < valorMejorEvalucionRecibida) {
					noSoyElMejor = true;
					hayEmpates = false;
					tengoLaMejorEvaluacion = false;
				} else if (mi_eval == valorMejorEvalucionRecibida) {
					tengoLaMejorEvaluacion = false;
					hayEmpates = true;
					noSoyElMejor = false;
		...
		// Si la evaluacion es -1 es decir esta fuera de rango ignoro la
		// evaluacion
		// el motor la elimina
	}

	public synchronized void addConfirmacionRealizacionObjetivo(
			AceptacionPropuesta confObjetivo) {
		// ArrayList p = paquete;
		// String eval = (String)p.get(0);
		String identObj = confObjetivo.getmsgAceptacionPropuesta();
		String idAgenteEmisorEvaluacion = confObjetivo.getIdentAgente();
		// trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
		// "Confirmacion Recibida. Dice Objetivo :" + identObj
		// +" Emisor: "+idAgenteEmisorEvaluacion, InfoTraza.NivelTraza.info));
		// actualizar las respuestas
		confirmacionesAgentes.set(
				agentesEquipo.indexOf(idAgenteEmisorEvaluacion), identObj);// guardamos
																			// la
																			// respuesta
	}

	public synchronized Integer calcularEvalucionParaDesempate() {
		// Incremento mi funcion de evaluacion con un numero aleatorio menor que
		// 2500
	...
	public synchronized ArrayList getRespuestas() {
		// mandar un mensaje a los agentes que no nos han enviado la respuesta
		// aun
	...
	public synchronized ArrayList<String> getAgentesEquipo() {
		// try {
		// // ArrayList agentesRegistrados =
		// itfConfig.getIdentificadoresInstanciasAgentesAplicacion();
		// // agentesEquipo = new ArrayList();
		// String aux;
		// for (int i = 0; i < agentesEquipo.size(); i++) {
		...
		// }
		// } catch (Exception ex) {
		// Logger.getLogger(InfoParaDecidirQuienVa.class.getName()).log(Level.SEVERE,
		// null, ex);
		// }
		return agentesEquipo;
	}
	...
	public synchronized ArrayList<String> getAgentesEmpatados() {
		// empezamos en el uno porque lo hemos inicializado en el cero
		// si esta vacio lo calculamos pero si ya hay elementos se devuelve como
		// esta. Esto se debe a que a terminado una rodo de desempate con
		// empates
	...
	public synchronized void setMiPropuestaParaAsumirElObjetivoEnviadaAtodos(
			Boolean valor) {
	...
	public synchronized void setMiDecisionParaAsumirElObjetivoEnviadaAtodos(
			Boolean valor) {
	...
	public synchronized void setheInformadoAlmejorParaQueAsumaElObjetivo(
			Boolean valor) {
	...
	public synchronized void setMiPropuestaDeDesempateEnviadaAtodos(
			Boolean valor) {
	...
	public String toString() {
		return " idElementoDecision->" + idElementoDecision
				...
				+ " ; equipo->" + agentesEquipo + " ; mi_eval->" + mi_eval
				...
				+ " ; hayEmpates->" + hayEmpates + " ; noSoyElMejor->"
				+ noSoyElMejor + " ; tengoAcuerdoDeTodos->"
				+ tengoAcuerdoDeTodos + " ; hayOtrosQueQuierenIr->"
				+ hayOtrosQueQuierenIr + " ; hanLlegadoTodasLasEvaluaciones->"
				+ hanLlegadoTodasLasEvaluaciones
				+ " ; numeroEvaluacionesRecibidas->"
				+ numeroEvaluacionesRecibidas
