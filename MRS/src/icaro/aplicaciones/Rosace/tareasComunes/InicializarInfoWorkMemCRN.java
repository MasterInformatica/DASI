...
public class InicializarInfoWorkMemCRN extends Tarea {
	...
	public void ejecutar(Object... params) {
		try {
			...
			this.getItfConfigMotorDeReglas()
					.setDepuracionActivationRulesDebugging(false);
			this.getItfConfigMotorDeReglas().setDepuracionHechosInsertados(
					false);
			this.getItfConfigMotorDeReglas().setDepuracionHechosModificados(
					false);
			this.getItfConfigMotorDeReglas()
			...
			identEquipo = this
					...
					.getValorPropiedadGlobal(
							NombresPredefinidos.NOMBRE_PROPIEDAD_GLOBAL_IDENT_EQUIPO);
			...
			if (miStatus != null) {
				ItfUsoMovimientoCtrl itfCompMov = (ItfUsoMovimientoCtrl) infoCompmov
						.getitfAccesoComponente();
				itfCompMov.inicializarInfoMovimiento(
						miStatus.getRobotCoordinate(),
				...
				this.trazas
						.trazar(miIdentAgte,
								"No se ha encontrado el fichero de inicializacion de Estatus",
	...
	private RobotStatus1 getRobotStatusInicial(String identRol) {
		...
		String rutaFicheroRobotsTest = AccesoPropiedadesGlobalesRosace
				.getRutaFicheroRobotsTest();
		// ReadXMLTestRobots rXMLTRobots = new
		// ReadXMLTestRobots(Constantes.rutasFicheroRobotsJerarquico);
		...
		Element info = null; // Lo siguienta habria que quitarlo cuando se
								// defina el recurso
		// String identAgteSinIdentEquipo =
		// miIdentAgte.replaceFirst(identEquipo, "");
		while (j < numeroRobotsSimulation && !encontrado) {
			...
			if (miIdentAgte.equals(valueid)) {
				encontrado = true; // Salir del bucle for. Se ha encontrado la
									// informacion xml asociada al robot/agente
									// que ejecuta esta tarea
			}
			j++;
		}
		if (encontrado) {
			...
			trazas.trazar(miIdentAgte,
					"No se ha podido encontrar el identificador del agente en el fichero :"
