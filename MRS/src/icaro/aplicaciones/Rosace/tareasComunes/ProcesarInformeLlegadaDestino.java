...
public class ProcesarInformeLlegadaDestino extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		try {
			...
			// habr�a que actualizar las victimas, los objetivos, el estado del
			// movimiento y cambiar el foco
			...
			// "Se Ejecuta la Tarea :"+ this.identTarea ,
			// InfoTraza.NivelTraza.info));
			trazas.aceptaNuevaTrazaEjecReglas(this.identAgente,
					"Se Procesa el informe   recibido por el agente :"
							+ informeRecibido.referenciaContexto
							+ " Cuyo contenido:"
							+ informeRecibido.contenidoInforme + "\n");
			// se actualiza el coste de la vicitima salvada
			itfcompMov = (ItfUsoMovimientoCtrl) infoCompMov
			...
			// Se actualizan los objetivos, se da por conseguido el objetivo
			// salvar a la victima
			// se supone que este objetivo era el mas prioritario, si no lo era
			// hay un problema
			Objetivo objetivoConseguido = misObjs.getobjetivoMasPrioritario();
			Thread accesoCompMovimiento = new Thread() {
				public void run() {
					itfcompMov.moverAdestino(victimaRescatar.getName(),
							victimaRescatar.getCoordinateVictim(),
							velocidadCruceroPordefecto);
				}
			};
			if (victimaRescatadaId.equals(objetivoConseguido
				...
				// Verificamos el foco si tiene un objetivo solved cambiamos el
				// foco al objetivo actual
				...
				if (nuevoObjetivo.getState() != Objetivo.SOLVED) {
					// tiene objetivos pendientes , se da la orden de que vaya a
					// salvar a la victima
					Victim victimaRescatada = victims
							.getVictimToRescue(victimaRescatadaId);
					String idVictimaRescatar = nuevoObjetivo
							.getobjectReferenceId();
					victimaRescatar = victims
							.getVictimToRescue(idVictimaRescatar);
					infoCompMov.setidentDestino(idVictimaRescatar);
					// infoCompMov.itfAccesoComponente.moverAdestino(nuevoObjetivo.getobjectReferenceId(),
					// victimaRescatada.getCoordinateVictim(),
					// velocidadCruceroPordefecto);
					accesoCompMovimiento.start();
				}
				String estadoComponente = EstadoMovimientoRobot.RobotEnMovimiento
						.name();
				// infoCompMov.setidentEstadoRobot(estadoComponente);
				// if(focoActual.getFoco().getState()==Objetivo.SOLVED)
				...
				trazas.aceptaNuevaTrazaEjecReglas(this.identAgente,
						"Se Ejecuta la Tarea :"
								...
								+ victimaRescatadaId
								+ " y el del ultimo objetivo : "
								+ objetivoConseguido.getobjectReferenceId()
								+ " coinciden "
								+ "EstadoComponente : "
								+ estadoComponente
								+ "Objetivo mas prioritario en curso "
								+ misObjs.getobjetivoMasPrioritario()
										.toString() + "\n"
								+ "  El foco esta en el ojetivo :  "
								+ focoActual + "\n");
			} else
				trazas.aceptaNuevaTrazaEjecReglas(this.identAgente,
						"Se Ejecuta la Tarea :" + this.identTarea
