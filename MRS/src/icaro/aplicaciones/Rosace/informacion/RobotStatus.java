...
public class RobotStatus {
	...
	// Actualmente en nuestra implementacion no se utilizan los atributos
	// rangeProximity y robotCapabilities.
	// No obstante esta clase ya ofrece metodos para poder considerarlos en el
	// futuro
	...
	public synchronized void setRobotCoordinate(Coordinate coord) {
		...
		if (infoCompMovt != null)
			infoCompMovt.itfAccesoComponente.setCoordenadasActuales(coord);
	}

	public synchronized Coordinate getRobotCoordinate() {
		if (sinMovimientoSignificativo() || infoCompMovt == null)
			return robotCoordinateActual;
		// if (infoCompMovt != null)
		return this.robotCoordinateActual = infoCompMovt.itfAccesoComponente
	...
	public boolean sinMovimientoSignificativo() {
		if (robotCoordinateAnterior == null)
			return false;
		return (limiteDespalzamiento >= Math.abs(robotCoordinateActual.getY()
				- robotCoordinateAnterior.getY()) && limiteDespalzamiento >= Math
				.abs(robotCoordinateActual.getX()
						- robotCoordinateAnterior.getX()));
	}

	@Override
	public String toString() {
		return "Robot: id->" + this.getIdRobot() + " ; Robot: Rol->"
				+ this.getIdRobotRol() + " ; engergylevel->"
				+ this.getAvailableEnergy() + " ; coordinate->"
				+ this.getRobotCoordinate() + " ; healrange->"
				+ this.getHealRange();
	}
}
/*
 * public synchronized boolean inicializar (String identEquipo, String
 * miIdentAgte, String identRol) { // Esto habria que cambiarlo cuando se defina
 * el Recurso String rutaFicheroRobotsTest =
 * AccesoPropiedadesGlobalesRosace.getRutaFicheroRobotsTest();
 * //ReadXMLTestRobots rXMLTRobots = new
 * ReadXMLTestRobots(Constantes.rutasFicheroRobotsJerarquico); ReadXMLTestRobots
 * rXMLTRobots = new ReadXMLTestRobots(rutaFicheroRobotsTest);
 * 
 * Document doc = rXMLTRobots.getDocument(rXMLTRobots.gettestFilePaht());
 * //Obtain all the robots NodeList nodeLst =
 * rXMLTRobots.getRobotsXMLStructure(doc, "robot"); int numeroRobotsSimulation =
 * rXMLTRobots.getNumberOfRobots(nodeLst); int j=0; boolean encontrado= false;
 * 
 * Element info = null ; // Lo siguienta habria que quitarlo cuando se defina el
 * recurso String identAgteSinIdentEquipo =
 * miIdentAgte.replaceFirst(identEquipo, ""); while(j<numeroRobotsSimulation &&
 * !encontrado ){ //Obtain info about robot located at the test info =
 * rXMLTRobots.getRobotElement(nodeLst, j); String valueid =
 * rXMLTRobots.getRobotIDValue(info, "id"); if
 * (identAgteSinIdentEquipo.equals(valueid)){ encontrado = true; //Salir del
 * bucle for. Se ha encontrado la informacion xml asociada al robot/agente que
 * ejecuta esta tarea } j++; }
 * 
 * if (encontrado){ int energy = rXMLTRobots.getRobotInitialEnergy(info,
 * "initialenergy"); Coordinate initialCoordinate =
 * rXMLTRobots.getRobotCoordinate(info); float healRange =
 * rXMLTRobots.getRobotHealRange(info, "healrange"); // RobotStatus robotStatus
 * = new RobotStatus(); this.idRobot = miIdentAgte ; this.idRobotRol=identRol;
 * this.availableEnergy = energy; this.robotCoordinate = initialCoordinate;
 * this.healRange = healRange; // return robotStatus ; }
 * 
 * return encontrado; } // public String toString(){ // return "Robot: id->" +
 * this.getIdRobot() + // " ; engergylevel->" + this.getAvailableEnergy() + //
 * " ; coordinate->" + this.getRobotCoordinate() + // " ; healrange->" +
 * this.getHealRange() + // " ; rangeproximity->" + this.getRangeProximity() +
