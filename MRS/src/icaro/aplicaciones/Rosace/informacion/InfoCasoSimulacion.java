...
public class InfoCasoSimulacion {
	private String identCaso; // Se podria poner una variable alfanumerica
								// seguida del tiempo inicial del caso
	...
	private Map<String, InfoAsignacionVictima> infoVictims2Rescue; // Victimas
																	// que hay
																	// en el
																	// entorno
	// private Map<String, String> victimasAsignadas = new HashMap <String,
	// String>();
	...
	private InfoSerieResultadosSimulacion seriePuntosAsignacion,
			seriePuntosLlegada, seriePuntosElapsed,
			serieInfoAsignacionVictimas;
	private InfoEntornoCasoSimulacion infoEntorno;
	private Set<String> victimasAsignadas;
	private boolean victimaNueva = false;

	// El tiempo del elemento i-esimo de elapsed es igual el tiempo del elemento
	// asignacion(i) - tiempo del elemento llegada(i)
	...
	public void setInfoCasoSimulacion(String equipoId, int numeroRobots,
			int numeroVictimas, int intervSecuencia) {
		infoEntorno = new InfoEntornoCasoSimulacion(equipoId, numeroRobots,
				numeroVictimas, intervSecuencia);
	...
	public void setInfoCasoSimulacion(InfoEntornoCasoSimulacion infEntorno) {
		...
		numeroVictimasDiferentesSimulacion = infoEntorno
				.getNrovictimasenentorno();
	...
	private void crearSeriesAconstruir() {
		...
		seriePuntosAsignacion = new InfoSerieResultadosSimulacion(
				VocabularioRosace.NombreFicheroSerieLlegadaYasignacion,
				infoEntorno);
		seriePuntosLlegada = new InfoSerieResultadosSimulacion(
				VocabularioRosace.NombreFicheroSeriePeticionVictimas,
				infoEntorno);
		seriePuntosElapsed = new InfoSerieResultadosSimulacion(
				VocabularioRosace.NombreFicheroSerieAsignacion, infoEntorno);
		serieInfoAsignacionVictimas = new InfoSerieResultadosSimulacion(
	...
	public void addInfoSerieIntervaloAsignacionVictimas() { // elapsed
		if (victimaNueva) {
			PuntoEstadistica puntoAsignacion = new PuntoEstadistica();
			double intervaloAsignacion = infoAsignacionVictima
					.getTiempoPeticion();
			puntoAsignacion.setX(numeroVictimasAsignadas);
			puntoAsignacion
					.setY((infoAsignacionVictima.getTiempoAsignacion() - infoAsignacionVictima
							.getTiempoPeticion()));
	...
	public InfoSerieResultadosSimulacion getSerieAsignacion() {
		seriePuntosAsignacion
				.setserieResultadosSimulacion(valoresSeriePuntosAsignacion);
		seriePuntosAsignacion
				.setnrovictimastotalasignadas(numeroVictimasAsignadas);
		return seriePuntosAsignacion;
	}

	public InfoSerieResultadosSimulacion getSerieLlegadaPeticiones() {
		seriePuntosLlegada
				.setserieResultadosSimulacion(valoresSeriePuntosLlegada);
		seriePuntosLlegada
				.setnrovictimastotalasignadas(numeroVictimasAsignadas);
	...
	public InfoSerieResultadosSimulacion getserieInfoAsignacionVictimas() {
		serieInfoAsignacionVictimas
				.setserieResultadosSimulacion(valoresSerieInfoAsignacionVictimas);
		serieInfoAsignacionVictimas
				.setnrovictimastotalasignadas(numeroVictimasAsignadas);
	...
	public InfoSerieResultadosSimulacion getSerieElapsed() {
		seriePuntosElapsed
				.setserieResultadosSimulacion(valoresSeriePuntosElapsed);
		seriePuntosElapsed
				.setnrovictimastotalasignadas(numeroVictimasAsignadas);
	...
	public boolean construirSeriesFromResultadosSimulacion(
			ArrayList<InfoAsignacionVictima> serieResultadosCasoSimulacion) {
		return false;
	}
}