/*
 *
 * Clase para gestionar las caracteristicas del robot en un escenario de simulaci�n :
 * 
...
public class RobotStatus1 implements Cloneable {
	...
	public Coordinate getRobotCoordinate() {
		if (robotCoordinate == null)
			robotCoordinate = new Coordinate(robotCoordinateActualP.x,
					robotCoordinateActualP.y, 0);
	...
	public RobotCapability getRobotCapability(String identCapab) {
		...
		boolean encontrado = false;
		while (i < robotCapabilities.size() & !encontrado) {
			if (robotCapabilities.get(i).getNombre()
					.equalsIgnoreCase(identCapab))
				encontrado = true;
			else
				i++;
		}
	...
	public boolean sinMovimientoSignificativo() {
		if (robotCoordinateAnteriorP == null)
			return false;
		return (limiteDespalzamiento >= Math.abs(robotCoordinateActualP.getY()
				- robotCoordinateAnteriorP.getY()) && limiteDespalzamiento >= Math
				.abs(robotCoordinateActualP.getX()
						- robotCoordinateAnteriorP.getX()));
	}
