...
public class ReadXMLTestRobots {
	...
	public synchronized Element getRobotElement(NodeList nodeLst,
	...
	// Obtain information about name for the current robot (contained in Element
	// info)
	...
	// Obtain information about initial energy for the current robot (contained
	// in Element info)
	...
	public synchronized int getRobotInitialEnergy(Element info, String tag) {
		...
		Element initialenergyNmElmnt = (Element) initialenergyNmElmntLst
				.item(0);
	...
	// Obtain information about x coordinate for the current robot (contained in
	// Element info)
	// tagcoordinate parameter should be equal to "initialcoordinate"
	// tagdimension parameter should be equal to "x"
	private synchronized String getRobotCoordinateX(Element info,
			String tagcoordinate, String tagdimension) {
		String valuex = "";
		NodeList coordinateNmElmntLst = info
				.getElementsByTagName(tagcoordinate);
	...
	// Obtain information about y coordinate for the current robot (contained in
	// Element info)
	// tagcoordinate parameter should be equal to "initialcoordinate"
	// tagdimension parameter should be equal to "y"
	private synchronized String getRobotCoordinateY(Element info,
			String tagcoordinate, String tagdimension) {
		String valuey = "";
		NodeList coordinateNmElmntLst = info
				.getElementsByTagName(tagcoordinate);
	...
	// Obtain information about z coordinate for the current robot (contained in
	// Element info)
	// tagcoordinate parameter should be equal to "initialcoordinate"
	// tagdimension parameter should be equal to "z"
	private String getRobotCoordinateZ(Element info, String tagcoordinate,
			String tagdimension) {
		String valuez = "";
		NodeList coordinateNmElmntLst = info
				.getElementsByTagName(tagcoordinate);
	...
	// Obtain information about coordinates for the current robot (contained in
	// Element info)
	public synchronized Coordinate getRobotCoordinate(Element info) {

		Coordinate robotCoordinate = new Coordinate(Double.parseDouble(this
				.getRobotCoordinateX(info, "initialcoordinate", "x")),
				Double.parseDouble(this.getRobotCoordinateY(info,
						"initialcoordinate", "y")), Double.parseDouble(this
						.getRobotCoordinateZ(info, "initialcoordinate", "z")));
		return robotCoordinate;
	}

	// Obtain information about heal range for the current robot (contained in
	// Element info)
	...
	public synchronized Coordinate getRobotCoordinate(String robotName) {
		...
		for (int j = 0; j < rXMLTRobots.getNumberOfRobots(nodeLst); j++) {
			...
			if (valueid.equals(robotName)) {
				Coordinate valueInitialCoordinate = rXMLTRobots
						.getRobotCoordinate(info);
				System.out
						.println("coordinate (Coordinate(double,double,double))-> "
								+ valueInitialCoordinate);
	...
	//
	// System.out.println("Nombre del elemento raiz " + documentElementName);
	// //sequence
	// System.out.println("Valor del elemento raiz " + documentElementValue);
	// //null
	//
	// //Obtain all the victims
	// NodeList nodeLst = doc.getElementsByTagName("victim");
	//
	// System.out.println("\nNumber de victims included in the sequence --> " +
	// nodeLst.getLength());
	//
	// System.out.println("\n==========================================");
	...
	// //__________________________________________________________________
	//
	// NodeList requirementsNmElmntLst =
	// info.getElementsByTagName("requirements");
	...
	// NodeList requirementNmElmntLst =
	// requirementsInfo.getElementsByTagName("requirement");
	//
	// //get the different requirements
	// for (int reqindex=0; reqindex < requirementNmElmntLst.getLength() ;
	// reqindex++){
